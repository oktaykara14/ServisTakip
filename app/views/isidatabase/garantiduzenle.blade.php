...
<div class="page-title">
    <h1>Isı Sayaç Parçaları <small>Bilgi Düzenleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/isidatabase/form-validation-9.js') }}"></script>
@stop
...
$(document).ready(function() {
            ...
            $.getJSON("{{ URL::to('isidatabase/sayacadicap') }}",{id:adiid}, function (event) {
                if (event.durum) {
                    if (event.capdurum === "1") {
                        ...
                        $('#sayaccap').val("");
                        $.getJSON("{{ URL::to('isidatabase/sayacparcalari') }}",{adiid:adiid,capid:1}, function (event) {
                            if (event.durum) {
    ...
    $('#sayaccap').on('change', function () {
        ...
        if (capid !== "") {
            ...
            $.getJSON("{{ URL::to('isidatabase/sayacparcalari') }}",{adiid:sayacadi,capid:capid}, function (event) {
                if (event.durum) {
                    var sayacparca = event.parcalar;
                    var r=[], j = -1;
                    var key,size;
        ...
        $.getJSON("{{ URL::to('isidatabase/sayacadicap') }}",{id:sayacadi}, function (event) {
            if (event.durum) {
                if (event.capdurum === "1") {
                    ...
                    if (sayaccap !== "") {
                        $.getJSON("{{ URL::to('isidatabase/sayacparcalari') }}",{adiid:sayacadi,capid:sayaccap,garanti:1}, function (event) {
                            if (event.durum) {
                                var sayacparca = event.parcalar;
                                var sayacgaranti = event.garantiler;
                                var r = [], j = -1;
                                var key,size;
                                r[++j] = '<thead><tr><th> # </th><th> Parça Adı </th><th> Garanti Süresi (yıl) </th></tr></thead>';
                                for (key = 0, size = sayacparca.length; key < size; key++) {
                                ...
                                $("#parcasayi").val(sayacparca.length);
                                $('#dataTable').html(r.join(''));
                                if (sayacgaranti === null) {
                                    var garanti = $('#spinner').spinner('value');
                                    for (key = 1, size = sayacparca.length; key <= size; key++) {
                                        $('#spinner' + key).spinner({value: garanti, min: 0, max: 5});
                                    }
                                } else {
                                    for (key = 0, size = sayacgaranti.length; key < size; key++) {
                                        $('#spinner' + (key + 1)).spinner({value: sayacgaranti[key], min: 0, max: 5});
                                    }
                                }
                            } else {
                                toastr[event.type](event.text, event.title);
                    ...
                    $('.kontrol').addClass("hide");
                    $('#sayaccap').val("");
                    $.getJSON("{{ URL::to('isidatabase/sayacparcalari') }}",{adiid:sayacadi,capid:1,garanti:1}, function (event) {
                        if (event.durum) {
                            var sayacparca = event.parcalar;
                            var sayacgaranti = event.garantiler;
                            var r = [], j = -1;
...
<div class="portlet box">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-pencil"></i>Sayaç Garanti Süresi Düzenle
        </div>
    </div>
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('isidatabase/garantiduzenle/'.$sayacgaranti->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="uretimyer" name="uretimyer" tabindex="-1" title="">
                            ...
                            @foreach($uretimyerleri as $uretimyeri)
                                @if((Input::old('uretimyer') ? Input::old('uretimyer') :$sayacgaranti->uretimyer_id)==$uretimyeri->id )
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="sayacadi" name="sayacadi" tabindex="-1" title="">
                            ...
                            @foreach($sayacadlari as $sayacadi)
                                @if((Input::old('sayacadi') ? Input::old('sayacadi') :$sayacgaranti->sayacadi_id)==$sayacadi->id )
                    ...
                    <div class="col-xs-8">
                        <select class="form-control select2me select2-offscreen" id="sayaccap" name="sayaccap" tabindex="-1" title="">
                            ...
                            @foreach($sayaccaplari as $sayaccapi)
                                @if((Input::old('sayaccap') ? Input::old('sayaccap') :$sayacgaranti->sayaccap_id)==$sayaccapi->id )
                ...
                <div class="form-group">
                    ...
                    <div id="spinner" class="col-xs-8">
                        <div class="input-group input-small">
                            <input type="text" name="spinner" value="{{ (Input::old('spinner') ? Input::old('spinner') :$sayacgaranti->garanti) }}" class="spinner-input form-control" maxlength="3" readonly="">
                            <div class="spinner-buttons input-group-btn btn-group-vertical">
                                <button type="button" class="btn spinner-up btn-xs blue">
            ...
            <div class="row">
                <div class="col-xs-12" style="text-align: center">
                    ...
                    <a href="{{ URL::to('isidatabase/sayacgaranti')}}" class="btn default">Vazgeç</a>
                </div>
            </div>
            </div>
        </form>
