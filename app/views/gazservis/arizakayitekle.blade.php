...
<div class="page-title">
    <h1>Gaz Arıza Kayıt <small>Ekleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/gazservis/form-validation-3.js') }}"></script>
@stop
    ...
    $(document).ready(function() {
        ...
        $('.getir').click(function () {
            ...
            if (serino !== "") {
                $.blockUI();
                var sayacgelenler;
                $.getJSON("{{ URL::to('gazservis/sayacbilgi') }}",{serino:serino}, function (event) {
                    if (event.durum === 0) // sayac bilgisini ekrana bas
                    {
                        ...
                        if (sayac.uretimyer_id === uretimyer.id) //bilgiler doğruysa
                        {
                            ...
                            $("#arizalar").valid();
                            $.getJSON("{{ URL::to('gazservis/sayacparca') }}",{sayacadiid:sayacgelen.sayacadi_id,sayaccapid:sayacgelen.sayaccap_id}, function (event) {
        ...
        if (serino !== "") {
            ...
            var sayacgelenler;
            if(sayacid !== "" ){
                $.getJSON("{{ URL::to('gazservis/sayacbilgi') }}",{serino:serino,sayacid:sayacid}, function (event) {
                    if (event.durum === 0) // sayac bilgisini ekrana bas
                    {
                        var sayacgelen = event.sayacgelenler[0];
                        var netsiscari = sayacgelen.netsiscari;
        ...
        $('#ekleneceklistesec').click(function () {
            ...
            var serino = $('#serino').val();
            $.blockUI();
            $.getJSON("{{ URL::to('gazservis/sayacgelenbilgi') }}",{serino:serino,sayacgelenid:sayacgelen_id}, function (event) {
        ...
        $('.serinoekle').click(function () {
            ...
            if (yeniserino === "") {
                ...
                $.blockUI();
                $.getJSON("{{ URL::to('gazservis/serinoekle') }}", {yeniserino: yeniserino,uretimyer:uretimyer,sayactur:3,sayacadi:sayacadi,sayaccap:sayaccap}, function (event) {
                    toastr[event.type](event.text, event.title);
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('gazservis/arizakayitekle') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate" enctype="multipart/form-data">
            <div class="form-body">
