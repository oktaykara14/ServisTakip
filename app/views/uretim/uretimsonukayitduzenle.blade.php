...
<div class="page-title">
    <h1>Üretim Sonu Kayıt <small>Düzenleme Ekranı</small></h1>
</div>
@stop
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.barkodguncelle').click(function () {
            ...
            var kayit = $('#kayitid').val();
            $.blockUI();
            $.getJSON("{{ URL::to('uretim/urunrecete') }}", {isemri: isemri,kayit:kayit,durum:true}, function (event) {
                if (event.durum) {
                    var recete = event.recete;
                    for(var i=0;i<recete.length;i++){
                        var barkodlar = recete[i].barkodlar;
            ...
            $.getJSON("{{ URL::to('uretim/urunrecete') }}", {isemri: isemri,kayit:kayit,durum:true}, function (event) {
                if (event.durum) {
                    ...
                    for(var i=0;i<recete.length;i++){
                                ...
                                if(eklibarkodlar[0]!==""){
                                    ...
                                    $.each(eklibarkodlar, function (j) {
                                        if(eklibarkodlar!=="") {
                                            ...
                                            var kullanilan = recete[i].uretimsonukullanilan[k];
                                            $('.barkod' + id + '_' + k).text(eklibarkodlar[j]);
                                            $('.barkodkalan' + id + '_' + k).text(kalan + '+' + kullanilan.urunadet);
                                            k++;
                                        }
                                    });
                                    $("#barkod" + (id)).select2("val",eklibarkodlar);
                        ...
                        if($( "#barkodlarekli"+(id)).hasClass( "barkodlarekli" ))
                        {
                            eklibarkodlar =document.getElementById("barkodlarekli"+(id)).innerHTML;
                            eklibarkodlar = eklibarkodlar.replace(/\s+/g,' ').trim();
                            eklibarkodlar = eklibarkodlar.split(",");
                            if(recete[i].kalem.length>0){
                                if(eklibarkodlar[0]!==""){
                                    ...
                                    adet = parseInt($('.barkodcount'+id).val());
                                    $('.barkodkalan'+id+'_0').text(adet);
                                    $("#barkod" + (id)).select2("val",eklibarkodlar);
                                }
                            }else {
        ...
        $('#kaydet').click(function () {
            ...
            $.getJSON("{{ URL::to('uretim/recetestokkontrol') }}", {stokkodlari: stokkodlari,depokodlari:depokodlari,adetler:adetler,durum:true,id:id}, function (event) {
                if (event.durum) {
...
<div class="portlet box">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-plus"></i>Üretim Sonu Kayıdı Düzenle
        </div>
    </div>
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('uretim/uretimsonukayitduzenle/'.$uretimsonukayit->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <label class="col-xs-8 isemri" style="padding-top: 7px">
                        {{ $isemri->ISEMRINO . ' - ' . $isemri->TARIH . ' Tarihli - ' . $isemri->MIKTAR .' Adet - ' .$isemri->STOK_KODU . ' - ' .$isemri->STOK_ADI . ($isemri->CARI_ISIM ? ' - ' . $isemri->CARI_ISIM : '') }}
                    </label>
                    <input type="text" id="kayitid" name="kayitid" value="{{Input::old('kayitid') ? Input::old('kayitid') : $uretimsonukayit->id}}" data-required="1" class="form-control hide">
                    <input type="text" id="isemri" name="isemri" value="{{Input::old('isemri') ? Input::old('isemri') : $isemri->ISEMRINO}}" data-required="1" class="form-control hide">
                </div>
                    ...
                    <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <div class="input-icon right col-xs-8">
                            ...
                            <select class="form-control select2me cikisdepo" id="cikisdepo" name="cikisdepo" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                @foreach($netsisdepolar as $depo)
                                    @if((Input::old('cikisdepo') ? Input::old('cikisdepo') : $uretimsonukayit->cikisdepo)==$depo->kodu )
                    ...
                    <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <div class="input-icon right col-xs-8">
                            ...
                            <select class="form-control select2me girisdepo" id="girisdepo" name="girisdepo" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                @foreach($netsisdepolar as $depo)
                                    @if((Input::old('girisdepo') ? Input::old('girisdepo') : $uretimsonukayit->girisdepo)==$depo->kodu )
                                        <option value="{{ $depo->kodu }}" selected>{{ $depo->kodu . ' - ' . $depo->adi }}</option>
                                    @else
                                        <option value="{{ $depo->kodu }}">{{ $depo->kodu . ' - ' . $depo->adi }}</option>
                                    @endif
                ...
                <div class="form-group col-xs-12">
                    ...
                    <div class="col-xs-12 col-sm-2 control-label" style="text-align: left;padding-top:0">
                        ...
                        <a class="btn blue serinouret {{(Input::old('isemri') ? Input::old('isemri') : $isemri->ISEMRINO)!="" ? "" : "hide"}}" data-toggle="modal" data-target="#serinouretim">SeriNo Üret </a>
                        <a class="btn red seritemizle {{$uretimsonukayit->serinolar!="" ? '' : 'hide'}}" >SeriNo Temizle </a>
                        <a class="btn yellow seriesle {{$uretimsonukayit->serinolar!="" ? '' : 'hide'}}" >SeriNo Eşleştir </a>
                    </div>
                </div>
                @if(Input::old('serino'))
                ...
                @else
                    <div id="serinoekli" class="hide serinoekli">{{ $uretimsonukayit->serinolar }}</div>
                @endif
                <div class="form-group col-xs-12">
                    <div class="form-group col-sm-6 col-xs-12">
                        <label class="control-label col-xs-4">Üretilen/ Üretilecek Miktar:</label>
                        <label class="col-xs-8 uretilecek" style="padding-top: 7px">{{Input::old('uretilecek') ? Input::old('uretilecek') : $uretimsonukayit->adet}} Adet</label>
                    </div>
                </div>
                <h4 class="form-section col-sm-12 ">Reçete Bilgisi</h4>

                <div class="form-group col-xs-12 hide">
                    <input type="text" id="count" name="count" value="{{Input::old('count') ? Input::old('count') : $recete->count()}}" data-required="1" class="form-control">
                    <input type="text" id="miktar" name="miktar" value="{{Input::old('miktar') ? Input::old('miktar') : $isemri->MIKTAR}}" data-required="1" class="form-control">
                    <input type="text" id="uretilen" name="uretilen" value="{{Input::old('uretilen') ? Input::old('uretilen') : $isemri->URETILENMIKTAR}}" data-required="1" class="form-control">
                    <input type="text" id="kalan" name="kalan" value="{{Input::old('kalan') ? Input::old('kalan') : $isemri->KALANMIKTAR}}" data-required="1" class="form-control">
                    <input type="text" id="uretilecek" name="uretilecek" class="form-control" value="{{Input::old('uretilecek') ? Input::old('uretilecek') : $uretimsonukayit->adet}}">
                    <input type="text" id="sonuretilen" name="sonuretilen" class="form-control" value="{{Input::old('sonuretilen') ? Input::old('sonuretilen') : $uretimsonukayit->sonuretilen}}">
                </div>
                <div class="panel-group accordion urunler col-xs-12" id="accordion1">
                    @for($i=0;$i<(Input::old('count') ? (int)(Input::old('count')) : $recete->count());$i++)
                        <div class="panel panel-default urunler_ek">
                            ...
                            <div class="panel-heading">
                                ...
                                <input name="stokkodu[]" class="stokkodu{{$i}} stokkodu hide" value="{{Input::old('stokkodu.'.$i) ? Input::old('stokkodu.'.$i) : $recete[$i]->HAM_KODU}}"/>
                                <input name="stokadi[]" class="stokadi{{$i}} hide" value="{{Input::old('stokadi.'.$i) ? Input::old('stokadi.'.$i) : $recete[$i]->HAMMADDE_ADI}}"/>
                                <input name="alturun[]" class="alturun{{$i}} hide" value="{{Input::old('alturun.'.$i) ? Input::old('alturun.'.$i) : $recete[$i]->kalem->count()>0 ? 1 : 0}}"/>
                            </div>
                            <div id="collapse_{{$i}}" class="panel-collapse in">
                                <div class="panel-body">
                                    <div class="form-group col-sm-6 col-xs-12">
                                        ...
                                        <div class="input-icon right col-xs-8 col-sm-6">
                                            ...
                                            <select class="form-control select2me valid{{$i}} depokodu depokodu{{$i}}" id="depokodu{{$i}}" name="depokodu[]" tabindex="-1" title="">
                                                <option value="">Seçiniz...</option>
                                                @foreach($netsisdepolar as $depo)
                                                    @if((Input::old('depokodu.'.$i) ? Input::old('depokodu.'.$i) : ($recete[$i]->uretimsonukullanilan->count()>0 ? $recete[$i]->uretimsonukullanilan->first()->depokodu : $uretimsonukayit->cikisdepo))==$depo->kodu)
                                                        <option value="{{ $depo->kodu }}" selected>{{ $depo->kodu . ' - ' . $depo->adi }}</option>
                                                    @else
                                                        <option value="{{ $depo->kodu }}">{{ $depo->kodu . ' - ' . $depo->adi }}</option>
                                                    @endif
                                    ...
                                    <div class="form-group col-sm-6 col-xs-12">
                                        ...
                                        <div class="input-icon right col-xs-8 col-sm-6">
                                            <i class="fa"></i>
                                            <input type="text" id="birimadet{{$i}}" name="birimadet[]" class="form-control birimadet hide" value="{{Input::old('birimadet.'.$i) ? Input::old('birimadet.'.$i) : $recete[$i]->MIKTAR}}">
                                            <input type="text" id="adet{{$i}}" name="adet[]" class="form-control adet valid{{$i}}" value="{{Input::old('adet.'.$i) ? Input::old('adet.'.$i) : $recete[$i]->kullanilanadet}}">
                                        </div>
                                    ...
                                    <div class="form-group col-xs-12 barkod_ek{{$i}}">
                                        ...
                                        <div class="form-group col-sm-6 col-xs-12">
                                            ...
                                            @else
                                                <div id="barkodlarekli{{$i}}" class="hide barkodlarekli">{{ $recete[$i]->secilenbarkodlar }}</div>
                                            @endif
                                        </div>
                                        <div class="form-group col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-xs-8">
                                                ...
                                                <label style="padding-top: 6px" class="barkodkullanilan{{$i}}_0 "></label>
                                            </div>
                                        </div>
                                        @if(intval(Input::old('alturun.'.$i) ? Input::old('alturun.'.$i) : $recete[$i]->kalem->count()>0)==0 && (Input::old('barkodcount.'.$i) ? Input::old('barkodcount.'.$i) : $recete[$i]->uretimsonukullanilan->count())>1)
                                            @for($j=1;$j<intval(Input::old('barkodcount.'.$i) ? Input::old('barkodcount.'.$i) : $recete[$i]->uretimsonukullanilan->count());$j++)
                                                <div class="form-group col-sm-6 col-xs-12">
                                                    <label class="col-xs-4 control-label">Barkod:<span class="required" aria-required="true"> * </span></label>
                                                    <div class="input-icon right col-xs-8">
                                                        <i class="fa"></i>
                                                ...
                                                <div class="form-group col-sm-6 col-xs-12">
                                                    ...
                                                    <div class="col-xs-8">
                                                        ...
                                                        <label style="padding-top: 6px" class="barkodkullanilan{{$i}}_{{$j}} "></label>
                                                    </div>
                                                </div>
                                            @endfor
                                        @endif
                ...
                <div class="form-group">
                    <div class="col-sm-6 col-xs-12 control-label barkodeklediv {{(Input::old('tumbarkod') ? Input::old('tumbarkod') : $uretimsonukayit->barkodlar)!="" ? 'hide' : ''}}" style="text-align: left;">
                        <a class="btn green barkodekle">Barkod Ekle </a>
                    </div>
                    <div class="form-group barkodbilgi {{(Input::old('tumbarkod') ? Input::old('tumbarkod') : $uretimsonukayit->barkodlar)!="" ? '' : 'hide'}}">
                        <div class="form-group col-xs-12">
                        ...
                        @else
                            <div id="barkodlarekli" class="hide barkodlarekli">{{ ''}}</div>
                        @endif
                        ...
                        @else
                            <div id="tumbarkodlarekli" class="hide tumbarkodlarekli">{{ $uretimsonukayit->barkodlar }}</div>
                        @endif
                    </div>
                </div>
