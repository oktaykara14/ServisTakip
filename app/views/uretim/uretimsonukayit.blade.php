...
<div class="page-title">
    <h1>Üretim <small>Sonu Kayıt Ekranı</small></h1>
</div>
@stop
...
//            $.ajax("/secure/url/for/sign-message?request=" + toSign).then(resolve, reject);
    ...
    <script>
        ...
        var oTable = table.DataTable({
            ...
            "bProcessing": true,
            "ajax": {
                "url": "{{ URL::to('uretim/uretimsonukayitlist') }}",
                "type": "POST",
                "data": {
                }
            },
            ...
            "fnDrawCallback" : function() {
                $(document).on("click", ".delete", function () {
                    var id = $(this).data('id');
                    $(".modal-footer #sayacid").attr('href',"{{ URL::to('uretim/uretimsonukayitsil') }}/"+id );
                });
            },
            "aaSorting": [[6,'desc']],
            ...
            "columns": [
                {data: 'id', name: 'uretimsonu.id',"class":"id","orderable": true, "searchable": true},
                {data: 'adi', name: 'netsisstokkod.adi',"orderable": true, "searchable": false},
                {data: 'kodu', name: 'netsisstokkod.kodu',"orderable": true, "searchable": false},
                {data: 'isemrino', name: 'uretimsonu.isemrino',"orderable": true, "searchable": true},
                {data: 'adet', name: 'uretimsonu.adet',"orderable": true, "searchable": true},
                {data: 'girisdepo', name: 'uretimsonu.girisdepo',"orderable": true, "searchable": true},
                {data: 'eklenmetarihi', name: 'uretimsonu.eklenmetarihi',"orderable": true, "searchable": false},
                {data: 'geklenmetarihi', name: 'uretimsonu.geklenmetarihi',"visible": false, "searchable": true},
                {data: 'nadi', name: 'netsisstokkod.nadi',"visible": false, "searchable": true},
        ...
        $('<label>Kriter: </label><select style="height: 34px;margin-left: 5px;border-radius: 4px;padding-top:2px;padding-right: 10px" id="kriter" tabindex="-1" title="" class="select2me">'+
            ...
            '<option value="8">Ürün Adı</option>'+
            '<option value="9">Stok Kodu</option>'+
            '<option value="3">İş Emri</option>'+
        ...
        $(document).ready(function() {
            ...
            $('.barkodbas').click(function () {
                ...
                if (id === "") {
                    ...
                    $.blockUI();
                    $.getJSON(" {{ URL::to('uretim/uretimsonuurunbilgi') }}", {id: id}, function (event) {
                        if (event.durum) {
                            var uretimsonu = event.uretimsonu;
                            var urunler = uretimsonu.urun;
                            $('.urunadi').text(uretimsonu.netsisstokkod.adi);
                            $('.urunstokkod').text(uretimsonu.netsisstokkod.kodu);
                            var barkod = $("#barkod");
                            var barkodlar = [];
                            for(var i = 0;i<urunler.length;i++){
                                barkod.append('<option value="' +urunler[i].serino + '"> ' + urunler[i].serino + '</option>');
                                barkodlar.push(urunler[i].serino);
                            }
                            barkod.select2("val", barkodlar);
                            $('#barkodadet').val(1);
            ...
            $('.barkodcikti').click(function () {
                ...
                var barkod = $('#barkod').select2('val');
                if (adet !== "" && parseInt(adet)>0) {
                    for(var i=0;i<barkod.length;i++){
                        setPrinter('zebra');
                        printZPL(barkod[i]);
                    }
                }
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-tag"></i>Sisteme Girilen Üretim Sonu Kayıtları
                    </div>
                    <div class="actions">
                        <a class="btn btn-default btn-sm barkodbas">
                            <i class="fa fa-print"></i> Barkod Çıktısı Al</a>
                        <a href="{{ URL::to('uretim/uretimsonukayitekle') }}" class="btn btn-default btn-sm">
                            <i class="fa fa-pencil"></i> Yeni Üretim Sonu Kayıdı Ekle </a>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Üretim Sonu Kayıdı Silinecek</h4>
                </div>
                <div class="modal-body">
                    Seçilen Üretim Sonu Kayıdını Silmek İstediğinizden Emin Misiniz?
                </div>
                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group col-xs-12">
                                <div class="form-group col-sm-6 col-xs-12">
                                    <label class="control-label col-xs-4">Ürün Adı:</label>
                                    <label class="col-xs-8 urunadi" style="padding-top: 7px"></label>
                                </div>
                                <div class="form-group col-sm-6 col-xs-12">
                                    <label class="control-label col-xs-4">Stok Kodu:</label>
                                    <label class="col-xs-8 urunstokkod" style="padding-top: 7px"></label>
                                </div>
                            </div>
                            <div class="form-group col-xs-12">
                                <div class="form-group col-sm-6 col-xs-12">
                                    ...
                                    <div class="col-xs-8">
                                        <select class="form-control select2 select2-offscreen hide" multiple="" tabindex="-1" id="barkod" name="barkod[]">
                                        </select>
                                        <input type="text" id="barkodadet" name="barkodadet" value="1" maxlength="3" data-required="1" class="form-control" readonly>
                                    </div>
                                </div>
                            </div>
                        </div>
