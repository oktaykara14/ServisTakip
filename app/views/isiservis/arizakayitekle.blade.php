...
<div class="page-title">
    <h1>Isı Arıza Kayıt <small>Ekleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/isiservis/form-validation-3.js') }}"></script>
@stop
...
var oTable = table.DataTable({
    ...
    "fnDrawCallback" : function() {
        $('.goster').click(function(){
            ...
            if(sayacid!=="") {
                $.blockUI();
                $.getJSON("{{ URL::to('isiservis/gelisbilgidetay') }}",{sayacid:sayacid,depoteslimid:depoteslimid}, function (event) {
                    if(event.durum){
    ...
    $(document).ready(function() {
        ...
        $('.getir').click(function () {
            ...
            if (serino !== "") {
                $.blockUI();
                var sayacgelenler;
                $.getJSON("{{ URL::to('isiservis/sayacbilgi') }}",{serino:serino}, function (event) {
                    if (event.durum === 0) // sayac bilgisini ekrana bas
                    {
                        ...
                        if (sayac.uretimyer_id === uretimyer.id) //bilgiler doğruysa
                        {
                            ...
                            $("#arizalar").valid();
                            $.getJSON("{{ URL::to('isiservis/sayacparca') }}",{sayacadiid:sayacgelen.sayacadi_id,sayaccapid:sayacgelen.sayaccap_id}, function (event) {
                            ...
                            $.getJSON("{{ URL::to('isiservis/serinokontrol') }}",{sayacid:sayac.id,serino:serino,uretimyerid:uretimyer.id,sayacyerid:sayac.uretimyer_id}, function (event) {
        ...
        $('.gelisbilgigetir').click(function () {
            ...
            if (sayacid !== "") {
                $.blockUI();
                $.getJSON("{{ URL::to('isiservis/gelisbilgi') }}",{sayacid:sayacid,cariid:cariid}, function (event) {
                    if(event.durum){
                        oTable.clear().draw();
        ...
        $('#sayacadi').on('change', function () {
            ...
            if(id!==""){
                ...
                if (capdurum === 0) //cap kontrol edilmiyor
                {
                    ...
                    if (id !== "") {
                        $.blockUI();
                        $.getJSON("{{ URL::to('isiservis/sayacparca') }}",{sayacadiid:id}, function (event) {
                            if (event.durum) //sayaç parçalarını listede göster
                            {
                                var parcalar = event.sayacparcalar.parca;
                                $("#degisenler").empty();
        ...
        $('#sayaccap').on('change', function () {
            ...
            if (sayacadiid !=="" && id !== "") {
                $.blockUI();
                $.getJSON("{{ URL::to('isiservis/sayacparca') }}",{sayacadiid:sayacadiid,sayaccapid:id}, function (event) {
                    if (event.durum) //sayaç parçalarını listede göster
                    {
                        var parcalar = event.sayacparcalar.parca;
                        $("#degisenler").empty();
        ...
        if (serino !== "") {
            ...
            var sayacgelenler;
            if(sayacid !== "" ){
                $.getJSON("{{ URL::to('isiservis/sayacbilgi') }}",{serino:serino,sayacid:sayacid}, function (event) {
                    if (event.durum === 0) // sayac bilgisini ekrana bas
                    {
                        var sayacgelen = event.sayacgelenler[0];
                        var netsiscari = sayacgelen.netsiscari;
        ...
        $('#ekleneceklistesec').click(function () {
            ...
            var serino = $('#serino').val();
            $.blockUI();
            $.getJSON("{{ URL::to('isiservis/sayacgelenbilgi') }}",{serino:serino,sayacgelenid:sayacgelen_id}, function (event) {
        ...
        $('.arizaekle').click(function () {
            ...
            if (ariza === "") {
                ...
                $.blockUI();
                $.getJSON(" {{ URL::to('isiservis/arizaekle') }}", {ariza: ariza}, function (event) {
                    toastr[event.type](event.text, event.title);
        ...
        $('.yapilanekle').click(function () {
            ...
            if (yapilan === "") {
                ...
                $.blockUI();
                $.getJSON(" {{ URL::to('isiservis/yapilanekle') }}", {yapilan: yapilan}, function (event) {
                    toastr[event.type](event.text, event.title);
        ...
        $('.uyariekle').click(function () {
            ...
            if (uyari === "") {
                ...
                $.blockUI();
                $.getJSON(" {{ URL::to('isiservis/uyariekle') }}", {uyari: uyari}, function (event) {
                    toastr[event.type](event.text, event.title);
        ...
        $('.hurdanedeniekle').click(function () {
            ...
            if (hurdaneden === "") {
                ...
                $.blockUI();
                $.getJSON(" {{ URL::to('isiservis/hurdanedeniekle') }}", {hurdaneden: hurdaneden}, function (event) {
                    toastr[event.type](event.text, event.title);
        ...
        $('.serinoekle').click(function () {
            ...
            if (yeniserino === "") {
                ...
                $.blockUI();
                $.getJSON("{{ URL::to('isiservis/serinoekle') }}", {yeniserino: yeniserino,uretimyer:uretimyer,sayactur:4,sayacadi:sayacadi,sayaccap:sayaccap}, function (event) {
                    toastr[event.type](event.text, event.title);
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('isiservis/arizakayitekle') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate" enctype="multipart/form-data">
            <div class="form-body">
            ...
            <div class="form-actions">
                <div class="row">
                    <div class="col-xs-12" style="text-align: center">
                        ...
                        <a href="{{ URL::to('isiservis/arizakayit')}}" class="btn default">Vazgeç</a>
                    </div>
                </div>
            </div>
        </form>
