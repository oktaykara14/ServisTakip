...
<div class="page-title">
    <h1>Isı Arıza Kayıt <small>Düzenleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/isiservis/form-validation-4.js') }}"></script>
@stop
    ...
    $(document).ready(function() {
        ...
        $('.serinoekle').click(function () {
            ...
            if (yeniserino === "") {
                ...
                $.blockUI();
                $.getJSON("{{ URL::to('isiservis/serinoekle') }}", {yeniserino: yeniserino,uretimyer:uretimyer,sayactur:4,sayacadi:sayacadi,sayaccap:sayaccap,sayacgelen:sayacgelen}, function (event) {
                    toastr[event.type](event.text, event.title);
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('isiservis/arizakayitduzenle/'.$arizakayit->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate" enctype="multipart/form-data">
            <div class="form-body">
                ...
                <div class="form-group col-sm-6 col-xs-12">
                    ...
                    <div class="input-icon right col-sm-8 col-xs-9">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="sayaccap" name="sayaccap" tabindex="5" title="" @if($arizakayit->sayaccap_id==1) disabled="" @endif>
                            <option value="">Seçiniz...</option>
                            @foreach($sayaccaplari as $sayaccapi)
                                @if((Input::old('sayaccap') ? Input::old('sayaccap') : $arizakayit->sayaccap_id)==$sayaccapi->id)
