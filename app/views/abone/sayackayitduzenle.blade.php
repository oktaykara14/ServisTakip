...
<div class="page-title">
    <h1>Sayaç Kayıt <small>Düzenleme Ekranı</small></h1>
</div>
@stop
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('abone/sayackayitduzenle/'.$sayackayit->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="kargoadi" name="kargoadi" tabindex="-1" title="">
                            ...
                            @foreach($kargofirmalari as $kargofirma)
                                @if((Input::old('kargoadi') ? Input::old('kargoadi') : $sayackayit->kargofirma_id )==$kargofirma->id )
                ...
                <div class="form-group col-sm-6 col-xs-12">
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><input type="text" id="belgeno" name="belgeno" value="{{Input::old('belgeno') ? Input::old('belgeno') : $sayackayit->belgeno}}" maxlength="15" data-required="1" class="form-control">
                    </div>
                </div>
                <div class="form-group col-sm-6 col-xs-12">
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="cariadi" name="cariadi" tabindex="-1" title="">
                            ...
                            @foreach($netsiscariler as $netsiscari)
                                @if((Input::old('cariadi') ? Input::old('cariadi') : $sayackayit->netsiscari_id)==$netsiscari->id )
                ...
                <div class="form-group col-sm-6 col-xs-12">
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><div class="input-group input-medium date date-picker" style="padding-left: 12px !important;" data-date-format="dd-mm-yyyy" data-date-viewmode="years">
                            <input id="gondermetarihi" type="text" name="gondermetarihi" class="form-control" value="{{Input::old('gondermetarihi') ? Input::old('gondermetarihi') : ($sayackayit->gondermetarihi ? date("d-m-Y", strtotime($sayackayit->gondermetarihi)) : '') }}">
                ...
                <div class="form-group col-sm-6  col-xs-12">
                    <label class="control-label col-xs-4" > Ekli Olan / Eklenecek Sayaçlar: </label>
                    <label class="col-xs-2 count" style="padding-top: 9px">{{Input::old('count') ? Input::old('count') : $sayackayit->adet .' Adet'}}</label>
                    <input type="text" id="count" name="count" value="{{Input::old('count') ? Input::old('count') : $sayackayit->adet}}" data-required="1" class="form-control hide">
                </div>
                <div class="form-group col-sm-6 col-xs-12">
                    ...
                    <div class="input-icon right col-xs-8">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="sayactur" name="sayactur" tabindex="-1" title="">
                            <option value="">Seçiniz...</option>
                            <option value="1" {{(Input::old('sayactur') ? Input::old('sayactur') : $sayackayit->sayactur_id)==1 ? 'selected' : ''}}>Ön Ödemeli Su Sayaçları</option>
                            <option value="2" {{(Input::old('sayactur') ? Input::old('sayactur') : $sayackayit->sayactur_id)==2 ? 'selected' : ''}}>Ön Ödemeli Elektrik Sayaçları</option>
                            <option value="3" {{(Input::old('sayactur') ? Input::old('sayactur') : $sayackayit->sayactur_id)==3 ? 'selected' : ''}}>Ön Ödemeli Gaz Sayaçları</option>
                            <option value="4" {{(Input::old('sayactur') ? Input::old('sayactur') : $sayackayit->sayactur_id)==4 ? 'selected' : ''}}>Ön Ödemeli Isı Sayaçları</option>
                            <option value="5" {{(Input::old('sayactur') ? Input::old('sayactur') : $sayackayit->sayactur_id)==5 ? 'selected' : ''}}>Mekanik Gaz Sayaçları</option>
                        </select>
                    </div>
                </div>
                <div class="panel-group accordion sayaclar col-xs-12" id="accordion1">
                    @for($i=0;$i<(int)(Input::old('count') ? Input::old('count') : $sayackayitbilgi->count());$i++)
                        <div class="panel panel-default sayaclar_ek"><input class="no hide" value="{{$i}}"/><input class="adet hide" value="1"/>
                            <div class="panel-heading">
                                <h4 class="panel-title">
                                    <a class="accordion-toggle accordion-toggle-styled" data-toggle="collapse" data-parent="#accordion1" href="#collapse_{{$i}}">{{Input::old('serino.'.$i.'') ? Input::old('serino.'.$i.'') : $sayackayitbilgi[$i]->serino}} </a>
                                </h4>
                            </div>
                            <div id="collapse_{{$i}}" class="panel-collapse in">
                                <div class="panel-body">
                                    <div class="form-group col-sm-6 col-xs-12">
                                        ...
                                        <div class="input-icon right col-xs-6 col-sm-8"><i class="fa"></i><input type="text" id="serino{{$i}}" name="serino[{{$i}}]" maxlength="20" class="form-control valid{{$i}} serino" value="{{Input::old('serino.'.$i.'') ? Input::old('serino.'.$i.'') : $sayackayitbilgi[$i]->serino}}" /></div>
                                    ...
                                    <div class="form-group col-sm-6 col-xs-12">
                                        ...
                                        <div class="input-icon right col-xs-8 col-sm-6">
                                            <i class="fa"></i><input type="text" id="arizaneden{{$i}}" name="arizaneden[{{$i}}]" class="form-control arizaneden" maxlength="500" value="{{Input::old('arizaneden.'.$i.'') ? Input::old('arizaneden.'.$i.'') : $sayackayitbilgi[$i]->arizanedeni}}">
                                        </div>
