...
<div class="page-title">
    <h1>Mekanik Gaz Sayaç Parçaları <small>Bilgi Düzenleme Ekranı</small></h1>
</div>
@stop
...
$(document).ready(function() {
    $('#sayacadi').on('change', function () {
        ...
        if (adiid !== "") {
            $.blockUI();
            $.getJSON("{{ URL::to('mekanikdatabase/sayacparcalari') }}",{adiid:adiid,capid:1}, function (event) {
                if (event.durum) {
    ...
    if (sayacadi !== "") {
        $.blockUI();
        $.getJSON("{{ URL::to('mekanikdatabase/sayacparcalari') }}",{adiid:sayacadi,capid:1}, function (event) {
            if (event.durum) {
                var sayacparca = event.parcalar;
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('mekanikdatabase/garantiduzenle/'.$sayacgaranti->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
