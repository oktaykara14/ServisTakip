...
@section('page-title')
    <div class="page-title">
        <h1>Mekanik Gaz Stok Hareketi <small>Düzenleme Ekranı</small></h1>
    </div>
@stop
...
@section('page-script')
    <script src="{{ URL::to('pages/mekanikdatabase/form-validation-11.js') }}"></script>
@stop
...
<script>
    $(document).ready(function() {
        ...
        if (parca !== "") {
            $.blockUI();
            $.getJSON("{{ URL::to('mekanikdatabase/stokadi') }}",{degisenid:parca}, function (event) {
                if (event.durum) {
        ...
        $('#parca').on('change', function () {
            ...
            if (id !== "") {
                $.blockUI();
                $.getJSON("{{ URL::to('mekanikdatabase/stokadi') }}",{degisenid:id}, function (event) {
                    if (event.durum) {
                        var netsisstokkod = event.netsisstokkod;
                        $(".stokadi").html(netsisstokkod.adi);
                    } else {
...
@section('content')
    <div class="portlet box">
        ...
        <div class="portlet-body form">
            <!-- BEGIN FORM-->
            <form action="{{ URL::to('mekanikdatabase/stokhareketduzenle/'.$stokgiriscikis->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
                <div class="form-body">
                ...
                <div class="form-actions">
                    <div class="row">
                        <div class="col-xs-12" style="text-align: center">
                            ...
                            <a href="{{ URL::to('mekanikdatabase/stokgirisi')}}" class="btn default">Vazgeç</a>
                        </div>
                    </div>
                </div>
            </form>
