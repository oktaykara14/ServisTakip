...
<div class="page-title">
    <h1>Mekanik Gaz <small>Sayaç Değişen Parçalar Bilgi Ekranı</small></h1>
</div>
@stop
...
@section('scripts')
    ...
    <script>
        ...
        var oTable = table.DataTable({
            ...
            "bProcessing": true,
            "ajax": {
                "url": "{{ URL::to('mekanikdatabase/degisenlerlist') }}",
                "type": "POST",
                "data": {
                }
            },
            ...
            "columns": [
                {data: 'id', name: 'degisenler.id',"class":"id","orderable": true, "searchable": true},
                {data: 'tanim', name: 'degisenler.tanim',"orderable": true, "searchable": false},
                {data: 'gparcadurum', name: 'degisenler.gparcadurum',"orderable": true, "searchable": false},
                {data: 'gstokkontrol', name: 'degisenler.gstokkontrol',"orderable": true, "searchable": false},
                {data: 'gsabit', name: 'degisenler.gsabit',"orderable": true, "searchable": false},
                {data: 'ntanim', name: 'degisenler.ntanim',"visible": false, "searchable": true},
                {data: 'nparcadurum', name: 'degisenler.nparcadurum',"visible": false, "searchable": true},
                {data: 'nstokkontrol', name: 'degisenler.nstokkontrol',"visible": false, "searchable": true},
                {data: 'nsabit', name: 'degisenler.nsabit',"visible": false, "searchable": true},
        ...
        $('<label>Kriter: </label><select style="height: 34px;margin-left: 5px;border-radius: 4px;padding-top:2px;padding-right: 10px" id="kriter" tabindex="-1" title="" class="select2me">'+
            ...
            '<option value="5">Değişen Parça Adı</option>'+
            '<option value="6">Parça</option>'+
            '<option value="7">Stok Kontrolü</option>'+
    ...
    $(document).on("click", ".delete", function() {
         var id = $(this).data('id');
         $(".modal-footer #sayacid").attr('href',"{{ URL::to('mekanikdatabase/degisensil') }}/"+id);
    });
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-tag"></i>Sayaç Değişen Parçalar
                    </div>
                    <div class="actions">
                        <a href="{{ URL::to('mekanikdatabase/degisenekle') }}" class="btn btn-default btn-sm">
                            <i class="fa fa-pencil"></i> Yeni Sayaç Değişen Parça Ekle </a>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Sayaç Değişen Parça Silinecek</h4>
                </div>
                <div class="modal-body">
                    Seçilen Sayaç Değişen Parça Tanımını Silmek İstediğinizden Emin Misiniz?
                </div>
