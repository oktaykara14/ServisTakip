...
<div class="page-title">
    <h1>Su Sayaç Garanti Süresi <small>Ekleme Ekranı</small></h1>
</div>
@stop
        ...
        $.getJSON(" {{ URL::to('sudatabase/sayacadicap') }}",{id:sayacadi}, function (event) {
            if (event.durum) {
                if (event.capdurum === "1") {
                    ...
                    if (sayaccap !== "") {
                        $.getJSON("{{ URL::to('sudatabase/sayacparcalari') }}",{adiid:sayacadi,capid:sayaccap}, function (event) {
                            if (event.durum) {
                                var sayacparca = event.parcalar;
                                var r = [], j = -1;
                                r[++j] = '<thead><tr><th> # </th><th> Parça Adı </th><th> Garanti Süresi (yıl) </th></tr></thead>';
                    ...
                    $('.kontrol').addClass("hide");
                    $('#sayaccap').val("");
                    $.getJSON(" {{ URL::to('sudatabase/sayacparcalari') }}",{adiid:sayacadi,capid:1}, function (event) {
                        if (event.durum) {
                            var sayacparca = event.parcalar;
                            var r = [], j = -1;
                            var key,size;
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('sudatabase/garantiekle') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
