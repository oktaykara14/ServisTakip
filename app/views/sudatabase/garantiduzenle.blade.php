...
<div class="page-title">
    <h1>Su Sayaç Parçaları <small>Bilgi Düzenleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/sudatabase/form-validation-9.js') }}"></script>
@stop
...
$(document).ready(function() {
            ...
            $.getJSON("{{ URL::to('sudatabase/sayacadicap') }}",{id:adiid}, function (event) {
                if (event.durum) {
                    if (event.capdurum === "1") {
                        ...
                        $('#sayaccap').val("");
                        $.getJSON("{{ URL::to('sudatabase/sayacparcalari') }}",{adiid:adiid,capid:1}, function (event) {
                            if (event.durum) {
    ...
    $('#sayaccap').on('change', function () {
        ...
        if (capid !== "") {
            ...
            $.getJSON("{{ URL::to('sudatabase/sayacparcalari') }}",{adiid:sayacadi,capid:capid}, function (event) {
                if (event.durum) {
                    var sayacparca = event.parcalar;
                    var r=[], j = -1;
                    var key,size;
        ...
        $.getJSON("{{ URL::to('sudatabase/sayacadicap') }}",{id:sayacadi}, function (event) {
            if (event.durum) {
                if (event.capdurum === "1") {
                    ...
                    if (sayaccap !== "") {
                        $.getJSON("{{ URL::to('sudatabase/sayacparcalari') }}",{adiid:sayacadi,capid:sayaccap,garanti:1}, function (event) {
                            if (event.durum) {
                                var sayacparca = event.parcalar;
                    ...
                    $('.kontrol').addClass("hide");
                    $('#sayaccap').val("");
                    $.getJSON("{{ URL::to('sudatabase/sayacparcalari') }}",{adiid:sayacadi,capid:1,garanti:1}, function (event) {
                        if (event.durum) {
                            var sayacparca = event.parcalar;
                            var sayacgaranti = event.garantiler;
                            var r = [], j = -1;
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('sudatabase/garantiduzenle/'.$sayacgaranti->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
            ...
            <div class="row">
                <div class="col-xs-12" style="text-align: center">
                    ...
                    <a href="{{ URL::to('sudatabase/sayacgaranti')}}" class="btn default">Vazgeç</a>
                </div>
            </div>
            </div>
        </form>
