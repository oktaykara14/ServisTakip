...
<div class="page-title">
    <h1>Ürün Listesi<small> Fiyat Kayıt Ekranı</small></h1>
</div>
@stop
...
@section('scripts')
    ...
    <script>
        ...
        var oTable = table.DataTable({
            ...
            "bProcessing": true,
            "ajax": {
                "url": "{{ URL::to('subedatabase/urunlist') }}",
                "type": "POST",
                "data": {
            ...
            "fnDrawCallback" : function() {
                $(document).on("click", ".delete", function () {
                    var id = $(this).data('id');
                    $(".modal-footer #id").attr('href',"{{ URL::to('subedatabase/urunsil') }}/"+id );
                });
            },
            "language": {
            ...
            "columns": [
                {data: 'id', name: 'subeurun.id',"class":"id","orderable": true, "searchable": true},
                {data: 'urunadi', name: 'subeurun.urunadi',"orderable": true, "searchable": false},
                {data: 'adi', name: 'sube.adi',"orderable": true, "searchable": false},
                {data: 'kodu', name: 'netsisstokkod.kodu',"orderable": true, "searchable": false},
                {data: 'adet', name: 'adet',"orderable": false, "searchable": false},
                {data: 'gdurum', name: 'subeurun.gdurum',"orderable": true, "searchable": false},
                {data: 'nurunadi', name: 'subeurun.nurunadi',"visible": false, "searchable": true},
                {data: 'nadi', name: 'sube.nadi',"visible": false, "searchable": true},
                {data: 'nkodu', name: 'netsisstokkod.nkodu',"visible": false, "searchable": true},
                {data: 'ndurum', name: 'subeurun.ndurum',"visible": false, "searchable": true},
        ...
        $('<label>Kriter: </label><select style="height: 34px;margin-left: 5px;border-radius: 4px;padding-top:2px;padding-right: 10px" id="kriter" tabindex="-1" title="" class="select2me">'+
            ...
            '<option value="6">Ürün Adı</option>'+
            '<option value="7">Şube Adı</option>'+
            '<option value="8">Stok Kodu</option>'+
            '<option value="9">Durumu</option>'+
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-tag"></i>Kayıtlı Ürün Listesi
                    </div>
                    <div class="actions">
                        <a href="{{ URL::to('subedatabase/urunekle') }}" class="btn btn-default btn-sm">
                            <i class="fa fa-pencil"></i> Yeni Ürün Ekle </a>
                    </div>
                </div>
