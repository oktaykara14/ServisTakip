...
<div class="page-title">
    <h1>Şube Sayaç <small>Ekleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/subedatabase/form-validation-5.js') }}"></script>
@stop
...
<script>
    $(document).ready(function() {
        ...
        $('.ekle').click(function(){
            ...
            var sayaccap = $('#genelsayaccap').select2('val');
            var sayaccapdurum = $('#genelsayacadi').select2().find(":selected").data("id");
            while( adet>0 ) {
                var newRow = "";
                var sayi = 0;
                while (adet > 0 && sayi < 100) {
                    newRow += '<div class="panel panel-default sayaclar_ek"><input class="no hide" value="'+(count)+'"/><input class="adet hide" value="1"/><div class="panel-heading"><h4 class="panel-title">' +
                        ...
                        '<div class="input-icon right col-xs-6 col-sm-8"><i class="fa"></i><input type="text" id="serino'+count+'" name="serino['+count+']" maxlength="10" class="form-control valid'+count+' serino" tabindex="'+(count+1)+'"></div>' +
        ...
        $('#genelsayacadi').on('change', function () {
            ...
            if(capdurum === 0) //cap kontrol edilmiyor
            {
                $("#genelsayaccap").select2("val", 1).trigger('change');
                $(".sayaccap").select2("val",1).trigger('change');
                $('.sayaccaplari').val(1);
                $("#genelsayaccap").prop("disabled", true);
                $(".sayaccap").prop("disabled", true);
            }else{
                $("#genelsayaccap").select2("val", "");
                $(".sayaccap").select2("val","").trigger('change');
                $('.sayaccaplari').val("");
                $("#genelsayaccap").prop("disabled", false);
                $(".sayaccap").prop("disabled", false);
            }
        });
        $('#genelsayaccap').on('change', function () {
            var id = $(this).val();
            $(".sayaccap").select2("val",id).trigger('change');
            $('.sayaccaplari').val(id);
        });
        $('input[name^="serino"]').change(function () {
            $(".kaydet").prop('disabled', false);
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('subedatabase/tektekekle') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                ...
                <div class="form-group col-xs-12">
                    ...
                    <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me" id="genelsayaccap" name="genelsayaccap" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                @foreach($sayaccaplari as $sayaccapi)
                                    @if(Input::old('genelsayaccap')==$sayaccapi->id)
                                        <option value="{{ $sayaccapi->id }}">{{ $sayaccapi->capadi }}</option>
                                    @else
                                        <option value="{{ $sayaccapi->id }}">{{ $sayaccapi->capadi }}</option>
                                    @endif
                            ...
                            <div id="collapse_{{$i}}" class="panel-collapse in">
                                <div class="panel-body">
                                    <div class="form-group col-xs-12">
                                        ...
                                        <div class="input-icon right col-xs-6 col-sm-8"><i class="fa"></i><input type="text" id="serino{{$i}}" name="serino[{{$i}}]" maxlength="10" class="form-control serino" value="{{Input::old('serino.'.$i.'')}}" tabindex="{{$i+1}}"/></div>
            ...
            <div class="form-actions">
                <div class="row">
                    <div class="col-xs-12" style="text-align: center">
                        ...
                        <a href="{{ URL::to('subedatabase/sayaclar')}}" class="btn default">Vazgeç</a>
                    </div>
                </div>
            </div>
        </form>
