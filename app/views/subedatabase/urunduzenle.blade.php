...
<div class="page-title">
    <h1>Ürün <small>Bilgi Düzenleme Ekranı</small></h1>
</div>
@stop
...
$(document).ready(function(){
    ...
    var sayaccapi=$('#sayaccapi').val();
    if(sayacadi!=="") {
        $("#sayacadi").select2("val",sayacadi).valid();
        ...
        if (capdurum === 0) //cap kontrol edilmiyor
        {
            ...
            $("#sayaccapi").select2("val",sayaccapi).valid();
...
<div class="portlet box">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-pencil"></i>Ürün Bilgi Düzenle
        </div>
    </div>
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('subedatabase/urunduzenle/'.$urun->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-xs-6">
                        <i class="fa"></i><input type="text" id="urunadi" name="urunadi" value="{{Input::old('urunadi') ? Input::old('urunadi') :$urun->urunadi }}" maxlength="100" data-required="1" class="form-control">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-icon right col-xs-6">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="stokkod" name="stokkod" tabindex="-1" title="">
                            ...
                            @foreach($netsisstokkodlari as $stokkod)
                                @if((Input::old('stokkod') ? Input::old('stokkod') :$urun->netsisstokkod_id)==$stokkod->id )
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-xs-offset-4 col-sm-8 col-xs-8" style="margin-top: 5px;font-size: 15px">
                        <input type="checkbox" id="kontrol" name="kontrol" class="kontrol" {{(Input::old('kontrol') ? Input::old('kontrol') : $urun->kontrol ) ? 'checked' : ''}}/>
                        Stok Kontrolü Olacak (Eksiye Düşme Olmayacak)
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-sm-offset-2 col-xs-offset-4 col-sm-8 col-xs-8" style="margin-top: 5px;font-size: 15px">
                        <input type="checkbox" id="baglanti" name="baglanti" class="baglanti" {{(Input::old('baglanti') ? Input::old('baglanti') : $urun->baglanti ) ? 'checked' : ''}}/>
                        Abone Sayacı ile Bağlantılı (Aboneye Satılan Sayaç ise Seçilecektir)
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-sm-offset-2 col-xs-offset-4 col-sm-8 col-xs-8" style="margin-top: 5px;font-size: 15px">
                        <input type="checkbox" id="ekstra" name="ekstra" class="ekstra" {{(Input::old('ekstra') ? Input::old('ekstra') : $urun->ekstra ) ? 'checked' : ''}}/>
                        Servis Kayıdında Kullanılacak (Ekstra Ücretlerde gözükecek)
                    </div>
                </div>
                <div class="form-group baglantisayac {{(Input::old('baglanti') ? Input::old('baglanti') : $urun->baglanti ) ? "" : "hide" }}">
                        ...
                        <div class="input-icon right col-xs-6">
                            <i class="fa"></i><select class="form-control select2me sayacadi " id="sayacadi" name="sayacadi" tabindex="-1" title="">
                                ...
                                @foreach($sayacadlari as $sayacadi)
                                    @if((Input::old('sayacadi') ? Input::old('sayacadi') : $urun->sayacadi_id  )==$sayacadi->id)
                        ...
                        <div class="input-icon right col-xs-6">
                            <i class="fa"></i> <select class="form-control select2me sayaccapi" id="sayaccapi" name="sayaccapi" tabindex="-1" title="">
                                ...
                                @foreach($sayaccaplari as $sayaccapi)
                                    @if((Input::old('sayaccapi')  ? Input::old('sayaccapi') : $urun->sayaccap_id )==$sayaccapi->id)
                ...
                <div class="form-group ekstra {{(Input::old('ekstra') ? Input::old('ekstra') : $urun->ekstra ) ? "" : "hide" }}">
                    <label class="control-label col-sm-2 col-xs-4">Fiyatı: <span class="required" aria-required="true"> * </span></label>
                    <div class="input-icon right col-xs-6">
                        <i class="fa"></i><input type="tel" id="fiyat" name="fiyat" value="{{Input::old('fiyat') ? Input::old('fiyat') :$urun->fiyat }}" data-required="1" class="form-control">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-icon right col-xs-6">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="parabirimi" name="parabirimi" tabindex="-1" title="">
                            ...
                            @foreach($parabirimleri as $parabirimi)
                                @if((Input::old('parabirimi') ? Input::old('parabirimi') :$urun->parabirimi_id)==$parabirimi->id )
                    ...
                    <div class="input-icon right col-xs-6">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="netsisdepo" name="netsisdepo" tabindex="-1" title="">
                            ...
                            @foreach($netsisdepolar as $netsisdepo)
                                @if((Input::old('netsisdepo') ? Input::old('netsisdepo') :$urun->depokodu)==$netsisdepo->kodu )
                ...
                <div class="form-group col-xs-12">
                    ...
                    <div class="col-xs-6">
                        <select class="form-control select2me durum" id="durum" name="durum" tabindex="-1" title="">
                            <option value="1" {{(Input::old('durum') ? Input::old('durum') : $urun->durum)==1 ? 'selected' : ''}}>Aktif</option>
                            <option value="0" {{(Input::old('durum') ? Input::old('durum') : $urun->durum)==0 ? 'selected' : ''}}>Pasif</option>
                        </select>
                    </div>
                </div>
