...
@section('page-title')
    ...
    <h1>Şube <small>Depo Stok Hareket Bilgi Ekranı</small></h1>
    </div>
@stop
...
@section('scripts')
    ...
    <script>
        ...
        var oTable = table.DataTable({
            ...
            "bProcessing": true,
            "ajax": {
                "url": "{{ URL::to('subedatabase/stokhareketlist') }}",
                "type": "POST",
                "data": {
            ...
            "fnDrawCallback" : function() {
                $(document).on("click", ".delete", function () {
                    var id = $(this).data('id');
                    $(".modal-footer #id").attr('href',"{{ URL::to('subedatabase/stokhareketsil') }}/"+id );
                });
            },
            "language": {
            ...
            "columns": [
                ...
                {data: 'db_name', name: 'stokgiriscikis.db_name',"orderable": true, "searchable": true},
                {data: 'fisno', name: 'stokgiriscikis.fisno',"orderable": true, "searchable": true},
                {data: 'gsecilenler', name: 'stokgiriscikis.gsecilenler',"orderable": true, "searchable": false},
                ...
                {data: 'nsecilenler', name: 'stokgiriscikis.nsecilenler',"visible": false, "searchable": true},
        ...
        $('<label>Kriter: </label><select style="height: 34px;margin-left: 5px;border-radius: 4px;padding-top:2px;padding-right: 10px" id="kriter" tabindex="-1" title="" class="select2me">'+
            ...
            '<option value="2">Fiş No</option>'+
            '<option value="8">İçeriği</option>'+
            '<option value="9">GCKod</option>'+
    ...
    <script>
        $(document).on("click", ".delete", function () {
            var id = $(this).data('id');
            $(".modal-footer #sayacid").attr('href',"{{ URL::to('subedatabase/stokhareketsil') }}/"+id);
        });
        $(document).ready(function() {
            $('#kriter').select2();
            $('.stokhareket').click(function () {
                var stokhareket = $('#sample_editable_1 .active .id').text();
                if (stokhareket !== "") {
                    $.extend({
                    ...
                    var redirect = "";
                    $.redirectPost(redirect, {stokhareket: stokhareket});
                }
            });
        });
    </script>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-tag"></i>Şube Depo Stok Hareketleri
                    </div>
                    <div class="actions">
                        <a class="btn btn-default btn-sm stokhareket">
                            <i class="fa fa-print"></i> Stok Hareket Çıktısı </a>
                        <a href="{{ URL::to('subedatabase/stokhareketekle') }}" class="btn btn-default btn-sm">
                            <i class="fa fa-pencil"></i> Yeni Stok Hareketi </a>
                    </div>
                </div>
