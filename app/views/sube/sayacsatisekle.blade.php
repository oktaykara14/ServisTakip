...
<div class="page-title">
    <h1>Şube Fatura <small>Ekleme Ekranı</small></h1>
</div>
@stop
...
<script>
    $(document).ready(function() {
        ...
        $('#nakit').maskMoney('mask',0);
        $('#kredikart').maskMoney('mask',0);
        $('#kredikart1').maskMoney('mask',0);
        ...
        $('.ekle').click(function(){
            ...
            $('.urunadi').on('change', function () {
                ...
                if(baglantidurum){
                    if(abone!==""){
                        $.blockUI();
                        $.getJSON("{{ URL::to('sube/abonesayac') }}",{id:abone},function(event){
                            if (event.durum) {
        ...
        $('.getir').click(function(){
            ...
            if (kriterdeger !== "" && kriter !== "" ) {
                $.blockUI();
                $.getJSON("{{ URL::to('sube/satisabonebilgi') }}",{tip:kriter,kriter:kriterdeger,subekodu:subekodu}, function (event) {
                    if (event.durum)
                    {
                        if(event.count>1){
        ...
        $('#listesec').click(function () {
            ...
            if ( aboneid !== "") {
                $.blockUI();
                $.getJSON("{{ URL::to('sube/satislistebilgigetir') }}", {id: aboneid,subekodu: subekodu}, function (event) {
                    if (event.durum) {
        ...
        var il = $('#faturail').val();
        var ilce = $('#faturailce').val();
        if(aboneid!==""){
            $.blockUI();
            $.getJSON("{{ URL::to('sube/abonesayac') }}",{id:aboneid,urun:1,il:il,ilce:ilce},function(event){
                if(event.durum){
                    ...
                    if(ilceler.length>0){
                        $("#faturailce").empty();
                        $("#faturail").select2("val",abone.iller_id);
                        ...
                        $("#faturailce").select2("val",abone.ilceler_id);
                    }
                    var ucretsizler=$('#ucretsizler').val();
        ...
        $('.urunadi').on('change', function () {
            ...
            if(baglantidurum){
                if(aboneid!==""){
                    $.blockUI();
                    $.getJSON("{{ URL::to('sube/abonesayac') }}",{id:aboneid},function(event){
                        if (event.durum) {
                            var sayaclar = event.abonesayaclari;
                            $("#abonesayac" + (no)).empty();
                            $.each(sayaclar, function (index) {
        ...
        $('#abone').on('change', function () {
            ...
            if(aboneid!==""){
                $.blockUI();
                $.getJSON("{{ URL::to('sube/abonesayac') }}",{id:aboneid,urun:1},function(event){
                    if(event.durum){
                        var sayaclar=event.abonesayaclari;
                        var urunler=event.urunler;
                        var abone=event.abone;
...
//            $(".kaydet").prop('disabled', true);
        ...
        $('#aboneformsubmit').click(function () {
            ...
            $.post("{{ URL::to('sube/hizliaboneekle') }}", {adisoyadi: adisoyadi, uretimyer:uretimyer, netsiscari:netsiscari, vergidairesi:vergidairesi,
                tckimlikno:tckimlikno, aboneno:aboneno, telefon:telefon,email:email, adres:adres, il:il, ilce:ilce, subekodu:subekodu, sayaclar:sayaclar }, function (event) {
                toastr[event.type](event.text, event.title);
                if(event.durum){
                    var abone = event.abone;
...
<div class="portlet box">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-plus"></i>Fatura Ekle
        </div>
    </div>
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('sube/sayacsatisekle') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="hide" id="abone" name="abone" value="{{ Input::old('abone')}}">
                ...
                <div class="form-group col-xs-12">
                    ...
                    <input class="hide" id="ucretsizler" name="ucretsizler" value="{{Input::old('ucretsizler')}}"/>
                    <input class="hide" id="baglantidurumlari" name="baglantidurumlari" value="{{Input::old('baglantidurumlari')}}"/>
                </div>
                                ...
                                <div id="collapse_{{$i}}" class="panel-collapse in">
                                    <div class="panel-body">
                                                ...
                                                <i class="fa"></i><select class="form-control select2me urunadi urunadi{{$i}}" id="urunadi{{$i}}" name="urunadi[]" tabindex="-1" title="">
                                                    @foreach($urunler as $urun)
                                                        @if((Input::old('urunadi.'.$i.''))==$urun->id)
                                                            <option data-id="{{ Input::old('gfiyat.'.$i.'') }}" data-fiyat="{{ Input::old('birimfiyat.'.$i.'') }}" data-baglanti="{{ $urun->baglanti }}" data-birim="{{ $urun->parabirimi_id }}" data-value="{{ $urun->parabirimi->birimi }}" value="{{ $urun->id }}" selected>{{ $urun->netsisstokkod->kodu.' - '.$urun->urunadi.'('.intval($urun->stok->BAKIYE).')'}}</option>
                                                        @else
                                                            <option data-id="{{ 0.00 }}" data-fiyat="{{ 0.00 }}" data-baglanti="{{ $urun->baglanti }}" data-birim="{{ $urun->parabirimi_id }}" data-value="{{ $urun->parabirimi->birimi }}" value="{{ $urun->id }}" >{{ $urun->netsisstokkod->kodu.' - '.$urun->urunadi.'('.intval($urun->stok->BAKIYE).')' }}</option>
                                                        @endif
                                                    @endforeach
                                        ...
                                        <div class="form-group col-sm-6 col-xs-12">
                                            ...
                                            <div class="input-icon right col-xs-8">
                                                <i class="fa"></i><input type="tel" id="fiyat{{$i}}" name="fiyat[]" class="form-control fiyat" value="{{Input::old('fiyat.'.$i.'')}}">
                                                <div class="hide">
                                                    <input type="text" id="birimfiyat{{$i}}" name="birimfiyat[]" class="form-control birimfiyat" value="{{Input::old('birimfiyat.'.$i.'')}}">
                                                    <input type="text" id="gfiyat{{$i}}" name="gfiyat[]" class="form-control gfiyat" value="{{Input::old('gfiyat.'.$i.'')}}">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-xs-4" style="margin-top: 5px;font-size: 15px">
                                                <input type="checkbox" id="ucretsiz{{$i}}" value="{{$i}}" name="ucretsiz[]" class="ucretsiz" {{(Input::old('ucretsizler') ? explode(',',Input::old('ucretsizler'))[$i] : '' ) ? 'checked' : ''}}/>Ücretsiz
                                            </div>
                                        </div>
                                        <div class="form-group baglanti baglantidurum{{$i}} hide">
                                            ...
                                            @endif
                                            <div class="hide">
                                                <input type="text" id="baglantidurum{{$i}}" name="baglantidurum[]" class="form-control" value="{{Input::old('baglantidurumlari') ? explode(',',Input::old('baglantidurumlari'))[$i] : ''}}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="form-group">
                    <div class="col-sm-3 col-xs-12 control-label" style="text-align: center;">
                        ...
                        <label class="control-label col-xs-10" style="text-align: center;"><span style="color:red" class="kurtarihi">Kur Tarihi: {{$dovizkuru[0]->tarih}}</span></label>
                    </div>
                    ...
                    <div class="col-sm-5 col-xs-12 control-label">
                        <div class="col-xs-12">
                            ...
                            <label class="col-xs-4 tutar" style="padding-top: 9px"><b>{{ Input::old('tutar') ? Input::old('tutar').' '.Input::old('birim') : '0.00 ₺' }}</b></label>
                        </div>
                        <div class="col-xs-12">
                            <label class="control-label col-xs-6"><b>KDV TUTARI:</b></label>
                            <label class="col-xs-4 kdvtutar" style="padding-top: 9px"><b>{{ Input::old('kdvtutar') ? Input::old('kdvtutar').' '.Input::old('birim') : '0.00 ₺' }}</b></label>
                        </div>
                        <div class="col-xs-12">
                            <label class="control-label col-xs-6"><b>TOPLAM TUTAR:</b></label>
                            <label class="col-xs-4 toplamtutar" style="padding-top: 9px"><b>{{ Input::old('toplamtutar') ? Input::old('toplamtutar').' '.Input::old('birim') : '0.00 ₺' }}</b></label>
                        </div>
                        <input type="text" id="tutar" name="tutar" class="form-control hide" value="{{ Input::old('tutar') ? Input::old('tutar') : 0 }}">
                        <input type="text" id="kdvtutar" name="kdvtutar" class="form-control hide" value="{{ Input::old('kdvtutar') ? Input::old('kdvtutar') : 0 }}">
                        <input type="text" id="toplamtutar" name="toplamtutar" class="form-control hide" value="{{ Input::old('toplamtutar') ? Input::old('toplamtutar') : 0 }}">
                        <input type="text" id="parabirimi" name="parabirimi" class="form-control hide" value="{{ Input::old('parabirimi') ? Input::old('parabirimi') : $parabirimi->id}}">
                        <input type="text" id="birim" name="birim" class="form-control hide" value="{{Input::old('birim') ? Input::old('birim') : $parabirimi->birimi}}">
                        <input type="text" id="kurtarih" name="kurtarih" class="hide" value="{{Input::old('kurtarih') ? Input::old('kurtarih') : $dovizkuru[0]->tarih}}">
                        <input type="text" id="geneltutar" name="geneltutar" class="form-control hide" value="{{ Input::old('geneltutar') ? Input::old('geneltutar') : 0 }}" >
                        <input type="text" id="genelkdv" name="genelkdv" class="form-control hide" value="{{ Input::old('genelkdv') ? Input::old('genelkdv') : 0 }}" >
                        <input type="text" id="geneltoplam" name="geneltoplam" class="form-control hide" value="{{ Input::old('geneltoplam') ? Input::old('geneltoplam') : 0 }}" >
                    </div>
                </div>
                ...
                <div class="form-group faturakismi">
                    <div class="form-group col-xs-12">
                        ...
                        <div class="input-icon right col-sm-10 col-xs-8">
                            <i class="fa"></i><input type="text" id="faturaadresi" name="faturaadresi" value="{{ Input::old('faturaadresi') }}" maxlength="100" data-required="1" class="form-control">
                        </div>
                    </div>
                    <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="faturailce" name="faturailce" tabindex="-1" title="">
                                ...
                                @foreach($ilceler as $ilce)
                                    @if(Input::old('faturailce')==$ilce->id )
                    ...
                    <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><input type="text" id="faturano" name="faturano" data-required="1" class="form-control" maxlength="15" placeholder="İrsaliye Sıra No">
                        </div>
                    </div>
                    <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me" id="odemesekli" name="odemesekli" tabindex="-1" title="">
                                <option data-id="0" {{Input::old('odemetipi')=="0" ? "selected" : ""}} value="">Seçiniz...</option>
                                <option data-id="1" {{Input::old('odemetipi')=="1" ? "selected" : ""}} value="NAKİT">NAKİT</option>
                                <option data-id="2" {{Input::old('odemetipi')=="2" ? "selected" : ""}} value="KREDİ KARTI">KREDİ KARTI</option>
                                <option data-id="3" {{Input::old('odemetipi')=="3" ? "selected" : ""}} value="SENET">SENET</option>
                                <option data-id="4" {{Input::old('odemetipi')=="4" ? "selected" : ""}} value="NAKİT+KREDİ KARTI">NAKİT + KREDİ KARTI</option>
                                <option data-id="6" {{Input::old('odemetipi')=="6" ? "selected" : ""}} value="2 KREDİ KARTI">2 FARKLI KREDİ KARTI</option>
                                <option data-id="5" {{Input::old('odemetipi')=="5" ? "selected" : ""}} value="BANKA HAVALESİ">BANKA HAVALESİ</option>
                                <option data-id="7" {{Input::old('odemetipi')=="7" ? "selected" : ""}} value="BELLİ DEĞİL">BELLİ DEĞİL (AÇIK FATURA)</option>
                            </select>
                        </div>
                        <div class="col-xs-4 hide">
                            <input type="text" id="odemetipi" name="odemetipi" class="form-control" value="{{Input::old('odemetipi')}}">
                        </div>
                    </div>
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me" id="kasakod" name="kasakod" tabindex="-1" title="">
                                ...
                                @foreach($kasakodlar as $kasakod)
                                    @if(Input::old('kasakod')==$kasakod->kasakod )
                    ...
                    <div class="form-group col-sm-6 col-xs-12 taksit hide">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me" id="taksit" name="taksit" tabindex="-1" title="">
                                <option data-id="0"     {{Input::old('taksit')=="1" ? "selected" : ""}} value="1">Tek Çekim</option>
                                <option data-id="6.15"  {{Input::old('taksit')=="2" ? "selected" : ""}} value="2">2 Taksit</option>
                                <option data-id="7.25"  {{Input::old('taksit')=="3" ? "selected" : ""}} value="3">3 Taksit</option>
                                <option data-id="8.35"  {{Input::old('taksit')=="4" ? "selected" : ""}} value="4">4 Taksit</option>
                                <option data-id="9.20"  {{Input::old('taksit')=="5" ? "selected" : ""}} value="5">5 Taksit</option>
                                <option data-id="10.10" {{Input::old('taksit')=="6" ? "selected" : ""}} value="6">6 Taksit</option>
                                <option data-id="11.20" {{Input::old('taksit')=="7" ? "selected" : ""}} value="7">7 Taksit</option>
                                <option data-id="11.90" {{Input::old('taksit')=="8" ? "selected" : ""}} value="8">8 Taksit</option>
                                <option data-id="12.55" {{Input::old('taksit')=="9" ? "selected" : ""}} value="9">9 Taksit</option>
                                <option data-id="13.55" {{Input::old('taksit')=="10" ? "selected" : ""}} value="10">10 Taksit</option>
                                <option data-id="14.55" {{Input::old('taksit')=="11" ? "selected" : ""}} value="11">11 Taksit</option>
                                <option data-id="15.25" {{Input::old('taksit')=="12" ? "selected" : ""}} value="12">12 Taksit</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group col-sm-6 col-xs-12 ikikredikarti hide">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me" id="kasakod2" name="kasakod2" tabindex="-1" title="">
                                <option data-id="0" value="">Seçiniz...</option>
                                @foreach($kasakodlar as $kasakod)
                                    @if(Input::old('kasakod2')==$kasakod->kasakod )
                                        <option data-id="{{$kasakod->odemetipi}}" value="{{ $kasakod->kasakod }}" selected>{{ $kasakod->carikod.' - '.$kasakod->kasaadi }}</option>
                                    @else
                                        <option data-id="{{$kasakod->odemetipi}}" value="{{ $kasakod->kasakod }}" disabled>{{ $netsiscari->carikod.' - '. $kasakod->kasaadi }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group col-sm-6 col-xs-12 ikikredikarti hide">
                        ...
                        <div class="input-icon right col-xs-8">
                            <i class="fa"></i><select class="form-control select2me" id="taksit2" name="taksit2" tabindex="-1" title="">
                                <option data-id="0"     {{Input::old('taksit2')=="1" ? "selected" : ""}} value="1">Tek Çekim</option>
                                <option data-id="6.15"  {{Input::old('taksit2')=="2" ? "selected" : ""}} value="2">2 Taksit</option>
                                <option data-id="7.25"  {{Input::old('taksit2')=="3" ? "selected" : ""}} value="3">3 Taksit</option>
                                <option data-id="8.35"  {{Input::old('taksit2')=="4" ? "selected" : ""}} value="4">4 Taksit</option>
                                <option data-id="9.20"  {{Input::old('taksit2')=="5" ? "selected" : ""}} value="5">5 Taksit</option>
                                <option data-id="10.10" {{Input::old('taksit2')=="6" ? "selected" : ""}} value="6">6 Taksit</option>
                                <option data-id="11.20" {{Input::old('taksit2')=="7" ? "selected" : ""}} value="7">7 Taksit</option>
                                <option data-id="11.90" {{Input::old('taksit2')=="8" ? "selected" : ""}} value="8">8 Taksit</option>
                                <option data-id="12.55" {{Input::old('taksit2')=="9" ? "selected" : ""}} value="9">9 Taksit</option>
                                <option data-id="13.55" {{Input::old('taksit2')=="10" ? "selected" : ""}} value="10">10 Taksit</option>
                                <option data-id="14.55" {{Input::old('taksit2')=="11" ? "selected" : ""}} value="11">11 Taksit</option>
                                <option data-id="15.25" {{Input::old('taksit2')=="12" ? "selected" : ""}} value="12">12 Taksit</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group col-xs-12 nakitkredikart hide">
                        ...
                        <div class="input-icon right col-xs-4">
                            <i class="fa"></i><input type="tel" id="nakit" name="nakit" data-required="1" class="form-control" maxlength="15" value="{{Input::old('nakit') ? Input::old('nakit').' '.Input::old('birim') : '0.00 ₺'}}" placeholder="Ne Kadar Nakit Geçilecek">
                        </div>
                        <label class="control-label col-xs-2">Kredi Kartı:<span class="required" aria-required="true"> * </span></label>
                        <label class="col-xs-4 kredikart" style="padding-top: 9px">{{Input::old('kredikart') ? Input::old('kredikart').' '.Input::old('birim') : '0.00 ₺'}}</label>
                        <div class="input-icon right col-xs-4 hide">
                            <i class="fa"></i><input type="tel" id="kredikart" name="kredikart" data-required="1" class="form-control" maxlength="15" value="{{Input::old('kredikart') ? Input::old('kredikart') : 0.00}}" placeholder="Ne Kadar Kredi Kartından Çekilecek">
                        </div>
                    </div>
                    <div class="form-group col-xs-12 ikikredikarti hide">
                        ...
                        <div class="input-icon right col-xs-4">
                            <i class="fa"></i><input type="tel" id="kredikart1" name="kredikart1" data-required="1" class="form-control" maxlength="15"  value="{{Input::old('kredikart1') ? Input::old('kredikart1').' '.Input::old('birim') : '0.00 ₺'}}" placeholder="Ne Kadar İlk Karttan Çekilecek">
                        </div>
                        <label class="control-label col-xs-2">2. Kredi Kartı:<span class="required" aria-required="true"> * </span></label>
                        <div class="input-icon right col-xs-4">
                            <i class="fa"></i><input type="tel" id="kredikart2" name="kredikart2" data-required="1" class="form-control" maxlength="15"  value="{{Input::old('kredikart2') ? Input::old('kredikart2').' '.Input::old('birim') : '0.00 ₺'}}" placeholder="Ne Kadar Diğer Karttan Çekilecek">
                        </div>
                        <input type="text" id="kredikartilk" name="kredikartilk" data-required="1" class="form-control hide" maxlength="15"  value="{{Input::old('kredikartilk') ? Input::old('kredikartilk') : 0}}">
                        <input type="text" id="kredikartilk2" name="kredikartilk2" data-required="1" class="form-control hide" maxlength="15" value="{{Input::old('kredikartilk2') ? Input::old('kredikartilk2') : 0}}">
                    </div>
                    <div class="form-group col-xs-12">
                        ...
                        <div class="input-icon right col-xs-10">
                            <i class="fa"></i><input type="text" id="aciklama" name="aciklama" data-required="1" class="form-control" maxlength="100" value="{{Input::old('aciklama')}}" placeholder="Seri Numaraları ya da Ücret Açıklaması( Tamir Bakım Ücreti,Smart Kart Bedeli vb.)">
                        </div>
                    </div>
                    <div class="form-group col-xs-12">
                        <label class="control-label col-xs-2">Ödemeyi Yapan:</label>
                        <div class="col-xs-6">
                            <input type="text" id="odemeyapan" name="odemeyapan" data-required="1" class="form-control" maxlength="100" value="{{Input::old('odemeyapan')}}" placeholder="Ödemeyi Yapan ile Fatura Sahibi Farklı ise Bilgi Amaçlı Eklenir">
                        </div>
                    </div>
                </div>
                    ...
                    <form id="form_sample_abone" class="form-horizontal" novalidate="novalidate">
                        <div class="form-body">
                            ...
                            <div class="form-group col-sm-6 col-xs-12">
                                ...
                                <div class="col-sm-8 col-xs-9">
                                    <input type="text" id="aboneemail" name="aboneemail" value="{{ Input::old('aboneemail') }}" maxlength="100" class="form-control">
                                </div>
                            </div>
                            <div class="form-group col-sm-6 col-xs-12">
                                <label class="control-label col-xs-4">İl:<span class="required" aria-required="true"> * </span></label>
