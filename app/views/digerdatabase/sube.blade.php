...
<div class="page-title">
    <h1>Şube <small> Bilgi Ekranı</small></h1>
</div>
@stop
...
@section('scripts')
    ...
    <script>
        ...
        var oTable = table.DataTable({
            ...
            "bProcessing": true,
            "ajax": {
                "url": "{{ URL::to('digerdatabase/subelist') }}",
                "type": "POST",
                "data": {
                }
            },
            ...
            "columns": [
                {data: 'id', name: 'sube.id',"class":"id","orderable": true, "searchable": true},
                {data: 'adi', name: 'sube.adi',"orderable": true, "searchable": false},
                {data: 'cariadi', name: 'netsiscari.cariadi',"orderable": true, "searchable": false},
                {data: 'subekodu', name: 'sube.subekodu',"orderable": true, "searchable": true},
                {data: 'kodu', name: 'netsisdepolar.kodu',"orderable": true, "searchable": true},
                {data: 'gaktif', name: 'sube.gaktif',"orderable": true, "searchable": false},
                {data: 'nadi', name: 'sube.nadi',"visible": false, "searchable": true},
                {data: 'ncariadi', name: 'netsiscari.ncariadi',"visible": false, "searchable": true},
                {data: 'naktif', name: 'sube.naktif',"visible": false, "searchable": true},
        ...
        $('<label>Kriter: </label><select style="height: 34px;margin-left: 5px;border-radius: 4px;padding-top:2px;padding-right: 10px" id="kriter" tabindex="-1" title="" class="select2me">'+
            ...
            '<option value="6">Şube Adı</option>'+
            '<option value="7">Cari Adı</option>'+
            '<option value="3">Şube Kodu</option>'+
            '<option value="4">Depo Kodu</option>'+
            '<option value="8">Durumu</option>'+
    ...
    <script>
        $(document).on("click", ".delete", function () {
            var id = $(this).data('id');
            $(".modal-footer #sayacid").attr('href',"{{ URL::to('digerdatabase/subesil') }}/"+id);
        });
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-tag"></i>Şubeler ve Depoları
                    </div>
                    <div class="actions">
                        <a href="{{ URL::to('digerdatabase/subeekle') }}" class="btn btn-default btn-sm">
                            <i class="fa fa-pencil"></i> Yeni Şube Ekle </a>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Şube Silinecek</h4>
                </div>
                <div class="modal-body">
                    Seçilen Şubeyi Silmek İstediğinizden Emin Misiniz?
                </div>
