...
<div class="page-title">
    <h1>Gaz Mekanik Arıza Kayıt <small>Detay Ekranı</small></h1>
</div>
@stop
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('mekanikgaz/arizakayitgoster/'.$arizakayit->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate" enctype="multipart/form-data">
           <div class="form-body">
                ...
                <div class="form-group col-xs-12">
                    <label class="control-label col-sm-2 col-xs-4">Aksesuar:</label>
                    <label class="col-sm-4 col-xs-8" style="padding-top: 9px">{{$arizakayit->aksesuarlar }} </label>
                    <label class="control-label col-sm-2 col-xs-4">Endeks:</label>
                    <label class="col-sm-4 col-xs-8" style="padding-top: 9px">{{round($arizakayit->ilkmekanik,3) }} </label>
                </div>
                <div class="form-group col-xs-12">
                    <label class="control-label col-sm-2 col-xs-4">Bağlantı Çapı:</label>
                    <label class="col-sm-4 col-xs-8" style="padding-top: 9px">{{  $arizakayit->baglanticap }} </label>
                    <label class="control-label col-sm-2 col-xs-4">Pmax:</label>
                    <label class="col-sm-4 col-xs-8" style="padding-top: 9px">{{  $arizakayit->pmax }} </label>
                </div>
                <div class="form-group col-xs-12">
                    <label class="control-label col-sm-2 col-xs-4">Qmax:</label>
                    <label class="col-sm-4 col-xs-8" style="padding-top: 9px">{{  $arizakayit->qmax }} </label>
                    <label class="control-label col-sm-2 col-xs-4">Qmin:</label>
                    <label class="col-sm-4 col-xs-8" style="padding-top: 9px">{{  $arizakayit->qmin }} </label>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-8">
                        <select multiple="multiple" class="multi-select" id="uyarilar" name="uyarilar[]">
                            @foreach($uyarilar as $uyari)
                                {{$flag=0}}
                                @foreach($arizakayit->uyarilar as $sonucuyari)
                                    @if($uyari->id==$sonucuyari)
                                        {{$flag=1}}
                ...
                <div class="form-group col-xs-12" style="margin:0;text-align:center">
                    ...
                    <label><input value="3" name="ozeldurum[]" type="checkbox" {{ ($arizakayit->arizakayit_durum)==3 ? 'checked' : '' }}  {{($arizakayit->arizakayit_durum)==3 ? '' : 'disabled' }}> Yedek Parça Bekliyor </label>
                    <label><input value="4" name="ozeldurum[]" type="checkbox" {{ ($arizakayit->arizakayit_durum)==4 ? 'checked' : '' }}  {{($arizakayit->arizakayit_durum)==4 ? '' : 'disabled' }}> Şikayetli Muayene </label>
                    <label><input value="5" name="ozeldurum[]" type="checkbox" {{ ($arizakayit->arizakayit_durum)==5 ? 'checked' : '' }}  {{($arizakayit->arizakayit_durum)==5 ? '' : 'disabled' }}> Müdahaleli Sayaç </label>
                    <label><input value="6" name="ozeldurum[]" type="checkbox" {{ ($arizakayit->arizakayit_durum==2 && $arizakayit->yenisayac) ? 'checked' : '' }}  {{($arizakayit->arizakayit_durum)==6 ? '' : 'disabled' }}> Yeni Sayaç Verildi </label>
                    <label><input value="7" name="ozeldurum[]" type="checkbox" {{ ($arizakayit->arizakayit_durum)==7 ? 'checked' : '' }}  {{($arizakayit->arizakayit_durum)==7 ? '' : 'disabled' }}> Geri İade </label>
                    <label><input value="8" name="ozeldurum[]" type="checkbox" {{ ($arizakayit->arizakayit_durum)==8 ? 'checked' : '' }}  {{($arizakayit->arizakayit_durum)==8 ? '' : 'disabled' }}> Geri İade(Kalibrasyonsuz) </label>
                </div>
