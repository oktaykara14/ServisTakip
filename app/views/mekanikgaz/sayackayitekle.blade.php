...
<div class="page-title">
    <h1>Gaz Mekanik Sayaç Kayıt <small>Ekleme Ekranı</small></h1>
</div>
@stop
...
@section('page-script')
<script src="{{ URL::to('pages/mekanikgaz/form-validation-5.js') }}"></script>
@stop
...
<script>
    $(document).ready(function() {
        ...
        $('.ekle').click(function(){
            ...
            '<div class="form-group col-sm-6 col-xs-12"><label class="col-xs-4 col-sm-4 control-label">Endeks:<span class="required" aria-required="true"> * </span></label>'+
            '<div class="input-icon right col-xs-6 col-sm-8"><i class="fa"></i><input type="text" id="endeks'+count+'" name="endeks['+count+']" class="form-control endeks" ></div></div>' +
            '</div></div></div>';
            count++;
            ...
            $("#endeks"+(count-1)).inputmask("decimal",{ radixPoint:",",groupSeparator: "",digits: 3,autoGroup: true});
            ...
            $('.satirsil').click(function() {
                if ($('.sayaclar .sayaclar_ek').size() > 0) {
                    ...
                    $('.sayaclar .sayaclar_ek').each(function () {
                        ...
                        $(this).children('div').children('div').children('div').children('div').children('.endeks').attr('id', 'endeks' + j).attr('name', 'endeks['+j+']');
        ...
        $('#cokluekle').click(function(){
            ...
            while( adet>0 ) {
                ...
                for (var i = ilksayi; i < sonsayi; i++) {
                    ...
                    $("#endeks" + (i)).inputmask("decimal",{ radixPoint:",",groupSeparator: "",digits: 3,autoGroup: true});
                }
                $("#count").val(count);
                $('input[name^="serino"]').change(function () {
                    $(".kaydet").prop('disabled', false);
                    ...
                    if (event.durum) {
                        ...
                        if(abonesayackayit.length>0){
                            ...
                            $('.detay').click(function(){
                                ...
                                if(kayitid!=="") {
                                    $.blockUI();
                                    $.getJSON("{{ URL::to('suservis/abonekayitdetay') }}",{kayitid:kayitid}, function (event) {
                                        if(event.durum){
                                            var abonesayackayit = event.abonesayackayit;
        ...
        $('#excelekle').click(function(){
            ...
            var hata = false;
            var type,text,title;
            if($('#exceluretimyer').select2("val") === ""){
                hata = true;
                type = "warning";
                title = "Gerekli Alanlardan Birisi Boş Geçildi!";
                text = "Geliş Yeri Boş Geçilemez!";
            }
            if(!hata){
                ...
                $.blockUI();
                $.ajax({
                    url:'{{ URL::to('mekanikgaz/sayackayitexcel') }}',
                    type :'POST',
                    dataType:'json',
                    data: formData,
                    async: true,
                    ...
                    success: function(event){
                        ...
                        if(!event.durum)
                            ...
                            while( adet>0 ){
                                ...
                                while(adet>0 && sayi<100) {
                                    newRow += '<div class="panel panel-default sayaclar_ek"><input class="no hide" value="'+(count)+'"/><input class="adet hide" value="1"/><div class="panel-heading"><h4 class="panel-title">'+
                                        ...
                                        '<option value="">Seçiniz...</option>'+
                                            ...
                                            @endforeach
                                                '</select><input type="text" id="sayaccaplari'+count+'" name="sayaccaplari[]" class="sayaccaplari hide" value="1"></div></div>'+
                                        '<div class="form-group col-sm-6 col-xs-12"><label class="col-xs-4 col-sm-4 control-label">Endeks:</label>'+
                                        '<div class="input-icon right col-xs-6 col-sm-8"><i class="fa"></i><input type="text" id="endeks'+count+'" name="endeks['+count+']" class="form-control endeks" value="'+degerler[inc].endeks+'"></div></div>' +
                                        '</div></div></div>';
                                    adet--;
                                    count++;
                                    sayi++;
                                ...
                                $("#exceluretimyer").select2("val","");
                                $("#excelsayacadi").select2("val","");
                                $("#excelistek").select2("val","");
                                $("#excel").val("");
                                $("select").on("select2-close", function () { $(this).valid(); });
                            }
                        }
        ...
        $('#excelsayacadi').on('change', function () {
            var id = $(this).val();
            if(id!==""){
                $.blockUI();
                $.getJSON("{{ URL::to('mekanikgaz/serviskod') }}",{sayacadiid:id}, function (event) {
                    if(event.durum){
                        var serviskod = event.serviskod;
                        $("#excelistek").select2('val',serviskod);
                    }else {
                        $('#excelekleme').modal('hide');
                        toastr[event.type](event.text, event.title);
                    }
                    $.unblockUI();
                });
        ...
        $('#listesec').click(function () {
            ...
            if ( kayitid !== "") {
                if(ekliolan!==kayitid){
                    ...
                    $.getJSON("{{ URL::to('mekanikgaz/abonekayitdetay') }}", {kayitid: kayitid}, function (event) {
                        if (event.durum) {
                            var abonesayackayit = event.abonesayackayit;
                            ...
                            var uretimtarih = $('#kayituretimtarih').select2('val');
                            var count = parseInt($("#count").val());
...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('mekanikgaz/sayackayitekle') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                        ...
                        <div class="panel panel-default sayaclar_ek"><input class="no hide" value="{{$i}}"/><input class="adet hide" value="1"/>
                            ...
                            <div id="collapse_{{$i}}" class="panel-collapse in">
                                <div class="panel-body">
                                    ...
                                    <div class="form-group col-sm-6 col-xs-12">
                                        <label class="col-xs-4 control-label">Üretim Yılı:</label>
                                        <div class="col-xs-8">
                                    ...
                                    <div class="form-group col-sm-6 col-xs-12">
                                        <label class="col-xs-4 col-sm-4 control-label">Endeks:<span class="required" aria-required="true"> * </span></label>
                                        <div class="input-icon right col-xs-6 col-sm-8">
                                            <i class="fa"></i><input type="text" id="endeks{{$i}}" name="endeks[{{$i}}]" class="form-control endeks" value="{{Input::old('endeks.'.$i.'')}}" >
                                        </div>
                                    </div>
                            </div>
                            </div>
            ...
            <div class="form-actions">
                <div class="row">
                    <div class="col-xs-12" style="text-align: center">
                        ...
                        <a href="{{ URL::to('mekanikgaz/sayackayit')}}" class="btn default">Vazgeç</a>
                    </div>
                </div>
            </div>
        </form>
                                ...
                                <form action="" id="form_sample_1" class="form-horizontal" novalidate="novalidate">
                                    <div class="form-body col-xs-12">
                                        ...
                                        <div class="form-group col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-xs-6">
                                                <select class="form-control select2me select2-offscreen" id="uretimtarih" name="uretimtarih" tabindex="-1" title="">
                                                    <option value="">Seçiniz...</option>
                                                    @foreach($yillar as $yil)
                            ...
                            <div class="portlet-body form">
                                <form action="" id="form_sample_2" class="form-horizontal" novalidate="novalidate" enctype="multipart/form-data">
                                    <div class="form-body col-xs-12">
                                        ...
                                        <h4>Sıra | Marka | Tip | Model | Sayaç No | İmalat Yılı | Endeks (Örneğin; 1 | ITRON | DYF | G4 | 12054334 | 2014 | 150 )</h4>
                                        <div class="form-group">
                                    ...
                                    <div class="form-body col-xs-12">
                                        <div class="portlet-body col-xs-12">
                                            ...
                                            <div class="form-group col-sm-6 col-xs-12">
                                                <label class="control-label col-xs-4">Üretim Tarihi:</label>
                                                <div class="col-xs-8">
                                                    <select class="form-control select2me select2-offscreen" id="kayituretimtarih" name="kayituretimtarih" tabindex="-1" title="">
                                                        <option value="">Seçiniz...</option>
                                                        @foreach($yillar as $yil)
                                                            @if(Input::old('kayituretimtarih')==$yil)
                                                                <option value="{{ $yil}}" selected>{{ $yil }}</option>
                                                            @else
                                                                <option value="{{ $yil }}" >{{ $yil }}</option>
                                                            @endif
