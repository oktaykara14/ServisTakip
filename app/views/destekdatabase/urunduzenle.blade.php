...
<div class="portlet box">
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('destekdatabase/urunduzenle/'.$urun->id.'') }}" id="form_sample" method="POST" enctype="multipart/form-data" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-xs-6">
                        <i class="fa"></i><input type="text" id="urun_adi" name="urun_adi" value="{{Input::old('urun_adi') ? Input::old('urun_adi') : $urun->urun_adi }}" data-required="1" class="form-control">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-sm-2 col-xs-4">Ürün Açıklaması:</label>
                    <div class="col-xs-8">
                        <textarea style="margin: 0;height: 180px;width: 500px;resize: none" name="aciklama">{{ Input::old('aciklama') ? Input::old('aciklama')  : $urun->aciklama }}</textarea>
                    </div>
                </div>
                <div class="form-group ">
                    <label class="control-label col-sm-2 col-xs-4">Ürün Resmi:</label>
                    <div class="col-xs-8">
                        <div class="fileinput {{is_null($urun->resim) ? 'fileinput-new' : 'fileinput-exists'}}" data-provides="fileinput">
                            <div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 200px; height: 150px;">
                                @if(!is_null($urun->resim))
                                    <img src='{{ URL::to('assets/urun/'.$urun->resim.'') }}' alt="urunresim"/>
                                @endif
                            </div>
                            <div>
