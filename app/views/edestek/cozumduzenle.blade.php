    ...
    if (options1 !== "") {
        $.getJSON(" {{ URL::to('edestek/detaylar') }}/" + options1, function (event) {
            ...
            if (detay.length > 0) {
                $("#options2").append('<option value="">Seçiniz...</option>');
                $.each(detay, function (index) {
                    if (detay[index].id === options2) {
                        $("#options2").append('<option value="' + detay[index].id + '" selected> ' + detay[index].detay + '</option>');
                    } else {
                        $("#options2").append('<option value="' + detay[index].id + '"> ' + detay[index].detay + '</option>');
                    }
                });
...
<div class="portlet box">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-pencil"></i>Hata Çözümü Düzenle
        </div>
    </div>
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('edestek/cozumduzenle/'.$hatacozum->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                    ...
                    <div class="input-icon right col-md-5 col-xs-12">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options1" name="options1" tabindex="-1" title="">
                            ...
                            @foreach($konular as $konu)
                                @if((Input::old('options1') ? Input::old('options1') : $hatacozum->edestekkonu_id)==$konu->id )
                    ...
                    <div class="input-icon right col-md-5 col-xs-12">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options2" name="options2" tabindex="-1" title="">
                            ...
                            @foreach($detaylar as $detay)
                                @if((Input::old('options2')? Input::old('options2') : $hatacozum->edestekkonudetay_id)==$detay->id )
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-md-7 col-xs-12">
                        <i class="fa"></i><input type="text" name="problem" value="{{Input::old('problem') ? Input::old('problem') : $hatacozum->problem }}" data-required="1" class="form-control">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-2 col-xs-12">Çözümü </label>
                    <div class="col-md-7 col-xs-12">
                        <textarea2 id="cozum" name="cozum">{{Input::old('cozumid') ? Input::old('cozumid') : $hatacozum->cozum }}</textarea2>
                    </div>
                </div>
                <input id="cozumid" name="cozumid" value='{{Input::old('cozumid') ? Input::old('cozumid') : $hatacozum->cozum }}' class="hide" />
