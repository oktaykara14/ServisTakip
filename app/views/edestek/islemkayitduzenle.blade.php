...
@section('page-title')
    <div class="page-title">
        <h1>İşlem Bilgisi <small>Düzenleme Ekranı</small></h1>
    </div>
@stop
...
@section('page-script')
    <script src="{{ URL::to('pages/edestek/form-validation-9.js') }}"></script>
@stop
...
$(document).ready(function() {
            ...
            $.getJSON(" {{ URL::to('edestek/konuislemler') }}/" + id, function (event) {
                ...
                if (islem.length > 0) {
                    ...
                    $.each(islem, function (index) {
                        $("#options3").append('<option value="' + islem[index].id + '"> ' + islem[index].islem + '</option>');
                    });
                } else {
                    $("#options3").empty();
                }
    ...
    if (options2 !== "") {
        $.getJSON(" {{ URL::to('edestek/konuislemler') }}/" + options2, function (event) {
            $("#options3").empty();
            var islem = event.islem;
            if (islem.length > 0) {
                $("#options3").append('<option value="">Seçiniz...</option>');
            ...
            $.getJSON(" {{ URL::to('edestek/konuislemekle') }}", {konu: konu, islemadi: islem}, function (event) {
                ...
                if (event.durum === 1 || event.durum === 2) {
                    ...
                    $.each(islemler, function (index) {
                        ...
                        $('#options3').append('<option value="' + islemler[index].id + '" >' + islemler[index].islem + '</option>');
                    });
...
@section('content')
    <div class="portlet box">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-plus"></i> İşlem Bilgisi Düzenle
            </div>
        </div>
        <div class="portlet-body form">
            <!-- BEGIN FORM-->
            <form action="{{ URL::to('edestek/islemkayitduzenle/'.$kayit->id ) }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
                <div class="form-body">
                        ...
                        <div class="input-icon right col-md-6 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options2" name="options2" tabindex="-1" title="">
                                ...
                                @foreach($konular as $konu)
                                    @if((Input::old('options2')? Input::old('options2') : $islem->edestekkonu_id)==$konu->id )
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon right col-md-6 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options3" name="options3" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                @foreach($konuislemler as $konuislem)
                                    @if((Input::old('options3')? Input::old('options3') : $islem->edestekkonuislem_id)==$konuislem->id )
                                        <option value="{{ $konuislem->id }}" selected>{{ $konuislem->islem }}</option>
                                    @else
                                        <option value="{{ $konuislem->id }}">{{ $konuislem->islem }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-7 col-xs-12">
                            <textarea2 id="islemdetay" name="islemdetay">{{Input::old('islemdetay')? Input::old('islemdetay') : $islem->detay }}</textarea2>
                        </div>
                    </div>
                    <input id="islemdetayid" name="islemdetayid" class="hide" value="{{Input::old('islemdetay')? Input::old('islemdetay') : $islem->detay }}" />
                    <div class="form-group">
                        ...
                        <div class="input-icon right col-md-7 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options4" name="options4" tabindex="-1" title="">
                                ...
                                @foreach($personeller as $personel)
                                    @if((Input::old('options4')? Input::old('options4') : $islem->edestekpersonel_id)==$personel->id )
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-4 col-xs-12">
                            <select class="form-control select2me select2-offscreen" id="options5" name="options5" tabindex="-1" title="">
                                <option value="0" @if((Input::old('options5')? Input::old('options5') : $islem->durum)=='0') selected @endif >Bekliyor</option>
                                <option value="1" @if((Input::old('options5')? Input::old('options5') : $islem->durum)=='1') selected @endif >Tamamlandı</option>
                            </select>
                        </div>
