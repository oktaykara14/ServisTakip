...
<div class="page-title">
    <h1>Kart Baskı Bilgisi <small>Düzenleme Ekranı</small></h1>
</div>
@stop
        ...
        $.getJSON(" {{ URL::to('edestek/musteribaski') }}/" + options1, function (event) {
            ...
            if (baski.length > 0) {
                ...
                $.each(baski, function (index) {
                    switch (baski[index].edestekbaskitur_id) {
                        case '1':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Su Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Su Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Su Kart Baskısı';
                            break;
                        case '2':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Kalorimetre Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Kalorimetre Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Kalorimetre Kart Baskısı';
                            break;
                        case '3':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Manas Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Manas Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Manas Kart Baskısı';
                            break;
                        case '4':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Trifaze Elektrik Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Trifaze Elektrik Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Trifaze Elektrik Kart Baskısı';
                            break;
                        case '5':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Monofaze Elektrik Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Monofaze Elektrik Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Monofaze Elektrik Kart Baskısı';
                            break;
                        case '6':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Baskısız </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Baskısız </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Baskısız';
                            break;
                        case '7':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Klimatik Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Klimatik Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Klimatik Kart Baskısı';
                            break;
                        case '8':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Doğalgaz Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Doğalgaz Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Doğalgaz Kart Baskısı';
                            break;
						case '9':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Mifare Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Mifare Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Mifare Kart Baskısı';
                            break;
						case '10':
                            if (options3 === baski[index].id)
                                $("#options3").append('<option value="' + baski[index].id + '" selected > Mifare Manas Kart Baskısı </option>');
                            else
                                $("#options3").append('<option value="' + baski[index].id + '"> Mifare Manas Kart Baskısı </option>');
                            document.getElementById("select2-chosen-3").innerHTML = 'Mifare Manas Kart Baskısı';
                            break;
                    }
        ...
        if (baskiid !== "" && baskiid !== null) {
            $.getJSON(" {{ URL::to('edestek/kartbaski') }}/" + baskiid, function (event) {
                switch (event.kartbaski.edestekbaskitur_id) {
					...
					case '8':
					case '9':
                        document.getElementById("baskion").src = "{{URL::to('assets/images/proje')}}/" + event.kartbaski.onresim;
                        document.getElementById("baskiarka").src = "{{URL::to('assets/images/proje')}}/" + event.kartbaski.arkaresim;
                        break;
                    case '3':
...
<div class="portlet box">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-pencil"></i>Kart Baskı Bilgisi Ekle
        </div>
    </div>
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{ URL::to('edestek/baskiduzenle/'.$kayit->id.'') }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
            <div class="form-body">
                    ...
                    <div class="input-icon right col-md-7 col-xs-12">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options2" name="options2" tabindex="-1" title="">
                            ...
                            @foreach($plasiyerler as $plasiyer)
                                @if((Input::old('options2')? Input::old('options2') : $baski->plasiyer_id)==$plasiyer->id )
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-md-8">
                        <i class="fa"></i><div class="input-group input-large date date-picker" style="padding-left: 12px !important;" data-date-format="dd-mm-yyyy" data-date-viewmode="years">
                             <input id="siparistarih" type="text" name="siparistarih" class="form-control" value="{{Input::old('siparistarih') ? Input::old('siparistarih') : date("d-m-Y", strtotime($baski->siparistarihi)) }}">
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-md-7 col-xs-12">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options3" name="options3" tabindex="-1" title="">
                            <option value="">Seçiniz...</option>
                            @foreach($baskilar as $baskitur)
                                @if((Input::old('options3')? Input::old('options3') : $baski->edestekkartbaski_id)==$baskitur->id )
                            <option value="{{ $baskitur->id }}" selected>{{ $baskitur->edestekbaskitur->adi }}</option>
                                @else
                            <option value="{{ $baskitur->id }}">{{ $baskitur->edestekbaskitur->adi }}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-md-7">
                        <i class="fa"></i><input type="text" id="mask_number" name="miktar" value="{{Input::old('miktar')? Input::old('miktar') : $baski->miktar}}" data-required="1" class="form-control">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-8">
                        <div class="input-group input-large date date-picker" style="padding-left: 12px !important;" data-date-format="dd-mm-yyyy" data-date-viewmode="years">
                                <input id="teslimtarih" type="text" name="teslimtarih" class="form-control" value="{{Input::old('teslimtarih') ? Input::old('teslimtarih') : date("d-m-Y", strtotime($baski->teslimtarihi)) }}">
                                <span class="input-group-btn">
                                <button class="btn default" type="button" style="border:1px solid #969696;padding-bottom: 5px;"><i class="fa fa-calendar"></i></button>
                                </span>
                        </div>
                    ...
                    <div class="input-icon right col-md-7 col-xs-12">
                        <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options4" name="options4" tabindex="-1" title="">
                            ...
                            @foreach($personeller as $personel)
                                @if((Input::old('options4')? Input::old('options4') : $baski->edestekpersonel_id)==$personel->id )
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon right col-md-2">
                        <i class="fa"></i><input type="text" id="sure" name="sure" value="{{Input::old('sure') ? Input::old('sure') : $baski->sure}}" class="form-control">
                    </div>
