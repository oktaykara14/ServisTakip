...
@section('page-title')
    <div class="page-title">
        <h1>Düzenli İşlem Bilgisi <small>Düzenleme Ekranı</small></h1>
    </div>
@stop
...
@section('page-script')
    <script src="{{ URL::to('pages/edestek/form-validation-8.js') }}"></script>
@stop
...
$(document).ready(function() {
    tinymce.init({
        ...
        setup: function (editor) {
            editor.on('change', function () {
                if (editor.id === "islemdetay") {
                    $("#islemdetayid").val(editor.getContent());
                } else {
                    $("#islemdetayyeniid").val(editor.getContent());
                }
            });
        }
    });
    ...
    $('#options3').on('change', function () {
        var id = $(this).val();
        $("#options4").select2("val", "");
        if (id !== "") {
            $.getJSON(" {{ URL::to('edestek/konuislemler') }}/" + id, function (event) {
                $("#options4").empty();
                var islem = event.islem;
                if (islem.length > 0) {
                    $("#options4").append('<option value="">Seçiniz...</option>');
                    $.each(islem, function (index) {
                        $("#options4").append('<option value="' + islem[index].id + '"> ' + islem[index].islem + '</option>');
                    });
                } else {
                    $("#options4").empty();
                }
    ...
    if (options3 !== "") {
        $.getJSON(" {{ URL::to('edestek/konuislemler') }}/" + options3, function (event) {
            $("#options4").empty();
            var islem = event.islem;
            if (islem.length > 0) {
                $("#options4").append('<option value="">Seçiniz...</option>');
    ...
    $('#islemekle').click(function () {
        ...
        if (konuid !== "") {
            $.getJSON(" {{ URL::to('edestek/konu') }}/" + konuid, function (event) {
                $(".konuislemyeni").text(event.konu.adi);
            });
        } else {
            $('#islem-ekle').modal('hide');
            toastr['warning']('Konu Seçilmedi', 'İşlem Ekleme Hatası');
        }
    });
    $('#islemduzenle').click(function () {
        //var konuid = $("#options3").val();
        var islemid = $("#options4").val();
        if (islemid !== "" && islemid !== null) {
            $.getJSON(" {{ URL::to('edestek/konuislem') }}/" + islemid, function (event) {
                $("#islemguncel").val(event.islem.islem);
                $(".konuislemguncel").text(event.islem.konu.adi);
            });
            $('#islem-duzenle').modal('show');
        } else {
            toastr['warning']('Yapılacak İşlem Seçilmedi', 'Düzenli İşlem Güncelleme Hatası');
        }
    });
            ...
            $.getJSON(" {{ URL::to('edestek/konuekle') }}", {konu: konu}, function (event) {
                ...
                if (event.durum === 1 || event.durum === 2) {
                    ...
                    $.each(konular, function (index) {
                        ...
                        $('#options3').append('<option value="' + konular[index].id + '" >' + konular[index].adi + '</option>');
                    });
                    $('#options3').select2('val', konuid);
                }
            });
        }
    });
    ...
    $('.islemekle').click(function () {
        var konu = $('#options3').val();
        var islem = $('#islemyeni').val();
        if (islem === "") {
            toastr['warning']('İşlem Boş Geçildi', 'İşlem Ekleme Hatası');
        } else {
            $.getJSON(" {{ URL::to('edestek/konuislemekle') }}", {konu: konu, islemadi: islem}, function (event) {
                toastr[event.type](event.text, event.title);
                $('#islem-ekle').modal('hide');
                if (event.durum === 1 || event.durum === 2) {
                    $('#options4').empty();
                    ...
                    $.each(islemler, function (index) {
                        if (islemler[index].islem === islem)
                            islemid = islemler[index].id;
                        $('#options4').append('<option value="' + islemler[index].id + '" >' + islemler[index].islem + '</option>');
                    });
                    $('#options4').select2('val', islemid);
                }
            });
        }
    });
    $('.islemduzenle').click(function () {
        ...
        if (islem === "") {
            ...
            $.getJSON(" {{ URL::to('edestek/konuislemduzenle') }}", {
                konu: konu,
                islemid: islemid,
                islem: islem
            }, function (event) {
    ...
    $('#baslangictarih').on('change', function() { $(this).valid(); });
});
</script>
@stop
...
@section('content')
    <div class="portlet box">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-plus"></i>Düzenli İşlem Bilgisi Düzenle
            </div>
        </div>
        <div class="portlet-body form">
            <!-- BEGIN FORM-->
            <form action="{{ URL::to('edestek/islemduzenle/'.$duzenliislem->id ) }}" id="form_sample" method="POST" class="form-horizontal" novalidate="novalidate">
                <div class="form-body">
                        ...
                        <div class="input-icon right col-md-6 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options1" name="options1" tabindex="-1" title="">
                                ...
                                @foreach($musteriler as $musteri)
                                    @if((Input::old('options1')? Input::old('options1') : $duzenliislem->edestekmusteri_id)==$musteri->id )
                    ...
                    <div class="form-group">
                        <label class="control-label col-md-2">Başlangıç Tarihi<span class="required" aria-required="true"> * </span></label>
                        <div class="input-icon right col-md-8">
                            <i class="fa"></i><div class="input-group input-large date date-picker" style="padding-left: 12px !important;" data-date-format="dd-mm-yyyy" data-date-viewmode="years">
                                <input id="baslangictarih" type="text" name="baslangictarih" class="form-control" value="{{Input::old('baslangictarih')? Input::old('baslangictarih') : date("d-m-Y", strtotime($duzenliislem->baslangictarih)) }}">
                    ...
                    <div class="form-group">
                        <label class="control-label col-md-2 col-xs-12">Aralığı <span class="required" aria-required="true"> * </span></label>
                        <div class="input-icon right col-md-1 col-xs-6">
                            <i class="fa"></i><input type="text" name="aralik" id="mask_number" value="{{Input::old('aralik')? Input::old('aralik') : $duzenliislem->aralik}}" data-required="1" class="form-control">
                        </div>
                        <div class="col-md-5 col-xs-6">
                            <select class="form-control select2me select2-offscreen" id="options2" name="options2" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                <option value="1" @if((Input::old('options2')? Input::old('options2') : $duzenliislem->araliktip)=='1') selected @endif >Gün</option>
                                <option value="2" @if((Input::old('options2')? Input::old('options2') : $duzenliislem->araliktip)=='2') selected @endif>Hafta</option>
                                <option value="3" @if((Input::old('options2')? Input::old('options2') : $duzenliislem->araliktip)=='3') selected @endif>Ay</option>
                                <option value="4" @if((Input::old('options2')? Input::old('options2') : $duzenliislem->araliktip)=='4') selected @endif>Yıl</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-icon right col-md-6 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options3" name="options3" tabindex="-1" title="">
                                ...
                                @foreach($konular as $konu)
                                    @if((Input::old('options3')? Input::old('options3') : $duzenliislem->edestekkonu_id)==$konu->id )
                    ...
                    <div class="form-group">
                        <label class="control-label col-md-2 col-xs-12">İşlem<span class="required" aria-required="true"> * </span></label>
                        <div class="input-icon right col-md-6 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options4" name="options4" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                @foreach($islemler as $islem)
                                    @if((Input::old('options4')? Input::old('options4') : $duzenliislem->edestekkonuislem_id)==$islem->id )
                                        <option value="{{ $islem->id }}" selected>{{ $islem->islem }}</option>
                                    @else
                                        <option value="{{ $islem->id }}">{{ $islem->islem }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4 col-xs-12">
                            <a href="#islem-ekle"  id="islemekle" data-toggle="modal" type="button" class="btn btn-success ">Ekle</a>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-7 col-xs-12">
                            <textarea2 id="islemdetay" name="islemdetay">{{Input::old('islemdetay')? Input::old('islemdetay') : $duzenliislem->detay }}</textarea2>
                        </div>
                    </div>
                    <input id="islemdetayid" name="islemdetayid" class="hide" value="{{Input::old('islemdetay')? Input::old('islemdetay') : $duzenliislem->detay }}" />
                    <div class="form-group">
                        ...
                        <div class="input-icon right col-md-7 col-xs-12">
                            <i class="fa"></i><select class="form-control select2me select2-offscreen" id="options5" name="options5" tabindex="-1" title="">
                                <option value="">Seçiniz...</option>
                                @foreach($personeller as $personel)
                                    @if((Input::old('options5')? Input::old('options5') : $duzenliislem->edestekpersonel_id)==$personel->id )
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-7 col-xs-12">
                            <select class="form-control select2me select2-offscreen" id="options6" name="options6" tabindex="-1" title="">
                                <option value="0" @if((Input::old('options6')? Input::old('options6') : $duzenliislem->durum)=='0') selected @endif >Pasif</option>
                                <option value="1" @if((Input::old('options6')? Input::old('options6') : $duzenliislem->durum)=='1') selected @endif >Aktif</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-md-2 col-xs-12">Son İşlem Tarihi</label>
                        <label class="col-md-5 col-xs-12" style="padding-top: 7px">{{ date("d-m-Y", strtotime($duzenliislem->sonislemtarihi)) }}</label>
                    </div>
                ...
                <div class="form-actions">
                    <div class="row">
                        <div class="col-xs-12" style="text-align: center">
                            ...
                            <a href="{{ URL::to('edestek/islemler')}}" class="btn default">Vazgeç</a>
                        </div>
                    </div>
                </div>
            </form>
                                ...
                                <form action="" id="form_sample_5" class="form-horizontal" novalidate="novalidate">
                                    <div class="form-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <label class="col-md-7 col-xs-12 konuislemyeni" style="margin-top: 9px"></label>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label col-md-2">Yapılacak İşlem <span class="required" aria-required="true"> * </span></label>
                                            <div class="input-icon right col-md-7">
                            ...
                            <div class="portlet-body form">
                                <form action="" id="form_sample_6" class="form-horizontal" novalidate="novalidate">
                                    <div class="form-body">
                                        ...
                                        <div class="form-group">
                                            <label class="control-label col-md-2 col-xs-12"></label>
                                            <label class="col-md-7 col-xs-12 konuislemguncel" style="margin-top: 9px"></label>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label col-md-2">Yapılacak İşlem <span class="required" aria-required="true"> * </span></label>
                                            <div class="input-icon right col-md-7">
