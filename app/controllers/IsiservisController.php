...
class IsiservisController extends BackendController {
    public $servisadi = 'isiservis';
    public $servisid = 4;
    ...
    public function getSayackayitekle() {
        ...
        return View::make($this->servisadi.'.sayackayitekle',array('uretimyerleri'=>$uretimyerleri,'netsiscariler'=>$netsiscariler,'sayacadlari'=>$sayacadlari,'sayaccaplari'=>$sayaccaplari,'servisstokkodlari'=>$servisstokkodlari))->with(array('title'=>$this->servisbilgi.' Sayaç Kayıdı Ekle'));
    }
        ...
        try {
            ...
            if (count($serinolar) > 0) {
                for ($i = 0; $i < count($serinolar); $i++) {
                    ...
                    if ($serino1 == "")
                        continue;
                    if(BackendController::SayacDurum($serino1,$yeri,$this->servisid,$subedurum,false,$sayacadi)){
                        Input::flash();
        ...
        try {
            ...
            $sayacgelenler = SayacGelen::select(array('sayacgelen.id', 'sayacgelen.depogelen_id', 'sayacgelen.netsiscari_id', 'sayacgelen.stokkodu',
                ...
                ->where('sayacgelen.servis_id', $this->servisid)->where(function ($query) {
                    $query->where('sayac.sayactur_id', $this->servisid)->orWhereNull('sayac.sayactur_id');
                })
    ...
    public function getCariyer()
    {
        try {
            ...
            $uretimyer = Uretimyer::whereIn('id', $cariyerler)->where('mekanik',0)->whereNotNull('oracle_id')->get();
