...
class MekanikdatabaseController extends BackendController {
    public $servisadi = 'mekanikdatabase';
    public $servisid = 5;
    ...
    public function postSayacadilist() {
        $query = SayacAdi::where('sayacadi.sayactur_id',$this->servisid)
            ->select(array("sayacadi.id","sayacadi.sayacadi","sayacmarka.marka","sayactip.tipadi","sayacadi.gbeyanname","sayacadi.nsayacadi","sayacmarka.nmarka","sayactip.ntipadi","sayacadi.nbeyanname"))
    ...
    public function getSayacadiekle() {
        ...
        $kalibrasyontipleri=KalibrasyonTipi::all();
        return View::make($this->servisadi.'.sayacadiekle',array('sayactipleri'=>$sayactipleri,'kalibrasyontipleri'=>$kalibrasyontipleri))->with(array('title'=>$this->servisbilgi.' Sayaç Tipi Ekle'));
    }

    public function postSayacadiekle() {
        try {
            $rules = ['adi' => 'required|unique:sayacadi,sayacadi', 'sayactip' => 'required', 'kalibrasyontip' => 'required'];
            $validate = Validator::make(Input::all(), $rules);
            $messages = $validate->messages();
            if ($validate->fails()) {
                Input::flash();
            ...
            $kalibrasyontip = Input::get('kalibrasyontip');
            $tur = $this->servisid;
            $cap = 0;
            If (Input::has('beyanname')) {
                $beyanname = 1;
            } else {
                $beyanname = 0;
            }
            ...
            $sayacadi->kalibrasyontipi_id = $kalibrasyontip;
            $sayacadi->cap = $cap;
            $sayacadi->beyanname = $beyanname;
            $sayacadi->save();
    ...
    public function getSayacadiduzenle($id) {
        ...
        $sayactipleri = SayacTip::where('sayactur_id',$this->servisid)->get();
        $kalibrasyontipleri=KalibrasyonTipi::all();
        return View::make($this->servisadi.'.sayacadiduzenle',array('sayacadi'=>$sayacadi,'sayactipleri'=>$sayactipleri,'kalibrasyontipleri'=>$kalibrasyontipleri))->with(array('title'=>$this->servisbilgi.' Sayaç Adı Düzenleme Ekranı'));
    }

    public function postSayacadiduzenle($id) {
        try {
            $rules = ['adi' => 'required|unique:sayacadi,sayacadi,' . $id, 'sayactip' => 'required', 'kalibrasyontip' => 'required'];
            $validate = Validator::make(Input::all(), $rules);
            $messages = $validate->messages();
            if ($validate->fails()) {
                Input::flash();
            ...
            $sayacadi->beyanname = $beyanname;
            $sayacadi->save();
            if(!BackendController::BeyannameGuncelle($sayacadi->id,$beyanname)){
                DB::rollBack();
                return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'Sayac Adı Güncellenemedi', 'text' => 'Sayaç Adını İçeren Gelen Sayaç Bilgileri Güncellenemedi.', 'type' => 'error'));
            }
    ...
    public function getFiyatekle() {
        $uretimyerleri = UretimYer::where('id',0)->orWhere('mekanik',1)->get();
    ...
    public function postSayaclist() {
        $query = Sayac::where('sayac.sayactur_id',$this->servisid)
            ->select(array("sayac.id","sayac.serino","sayacadi.sayacadi","uretimyer.yeradi","sayac.uretimtarihi","sayac.guretimtarihi","sayacadi.nsayacadi",
                "uretimyer.nyeradi"))
    ...
    public function postTektekekle() {
        try {
            ...
            BackendController::IslemEkle(1, Auth::user()->id, 'label-success', 'fa-plus-circle', 'Mekanik Gaz İçin Yeni Sayaç Seri Noları Eklendi.', 'Ekleyen:' . Auth::user()->adi_soyadi . ',Seri Nolar:' . $adet>10 ? $adet.' Adet' : $eklenenler);
            DB::commit();
            if ($flag == 1) {
    ...
    public function postSayacduzenle($id) {
        try {
            ...
            if ($sayac) {
                ...
                $sayac->save();
                BackendController::IslemEkle(2, Auth::user()->id, 'label-warning', 'fa-edit', 'Mekanik Gaz İçin ' . $serino . ' Seri Nolu Sayacın Bilgileri Güncellendi.', 'Güncelleyen:' . Auth::user()->adi_soyadi . ',Sayaç Numarası:' . $bilgi->id);
                DB::commit();
    ...
    public function getSayacsil($id){
        try {
            ...
            if($sayac){
                ...
                $sayac->delete();
                BackendController::IslemEkle(3, Auth::user()->id, 'label-danger', 'fa-close', 'Mekanik Gaz İçin ' . $bilgi->serino . ' Seri Nolu Sayacın Bilgileri Silindi.', 'Silen:' . Auth::user()->adi_soyadi . ',Sayaç Numarası:' . $bilgi->id);
                DB::commit();
    ...
    public function postUretimyerilist() {
        $query = UretimYer::where('mekanik',1)->select(array("uretimyer.id","uretimyer.yeradi","parabirimi.gbirimi","uretimyer.nyeradi","parabirimi.nbirimi"))
    ...
    public function postUretimyeriekle() {
        try {
            ...
            DB::beginTransaction();
            If (UretimYer::onlyTrashed()->where('yeradi', $adi)->where('mekanik', 1)->first()) {
                try {
                    $uretimyer = UretimYer::onlyTrashed()->where('yeradi', $adi)->where('mekanik', 1)->first();
            ...
            If (UretimYer::where('yeradi', $adi)->where('mekanik', 1)->first()) {
                DB::rollBack();
    ...
    public function postUretimyeriduzenle($id) {
        try {
            ...
            $birim = Input::get('birim');
            If (UretimYer::where('yeradi', $adi)->where('mekanik', 1)->where('id', '<>', $id)->first()) {
                DB::rollBack();
                Input::flash();
                return Redirect::back()->withInput()->with(array('mesaj' => 'true', 'title' => 'Doğrulama Hatası', 'text' => 'Bu Üretim yeri mevcut.', 'type' => 'warning'));
            }
    ...
    public function getIstasyon() {
        return View::make($this->servisadi.'.istasyon')->with(array('title'=>'Kalibrasyon İstasyon Bilgileri'));
    }

    public function postIstasyonlist() {
        $query = Istasyon::select(array("istasyon.id","istasyon.istasyonadi","istasyon.sayacsayi","istasyon.sayacadlari","istasyon.nistasyonadi"));
        return Datatables::of($query)
            ->editColumn('sayacadlari', function ($model) {
                return BackendController::getSayacAdlari($model->sayacadlari);
            })
            ->addColumn('islemler', function ($model) {
                $root = BackendController::getRootDizin();
                return "<a class='btn btn-sm btn-warning' href='".$root."/".$this->servisadi."/istasyonduzenle/".$model->id."'> Düzenle </a>".
                "<a href='#portlet-delete' data-toggle='modal' data-id='".$model->id."' class='btn btn-sm btn-danger delete' data-original-title='' title=''>Sil</a>";
            })
            ->make(true);
    }

    public function getIstasyonekle() {
        $sayacadlari = SayacAdi::where('sayactur_id',$this->servisid)->get();
        return View::make($this->servisadi.'.istasyonekle',array('sayacadlari'=>$sayacadlari))->with(array('title'=>$this->servisadi.' Kalibrasyon İstasyonu Ekle'));
    }

    public function postIstasyonekle() {
        try {
            $rules = ['istasyonadi' => 'required', 'sayacsayi' => 'required', 'sayacadi' => 'required'];
            $validate = Validator::make(Input::all(), $rules);
            $messages = $validate->messages();
            if ($validate->fails()) {
                Input::flash();
            ...
            DB::beginTransaction();
            $istasyon = new Istasyon;
            $adi = Input::get('istasyonadi');
            $sayi = Input::get('sayacsayi');
            if(Istasyon::where('istasyonadi',$adi)->first()){
                Input::flash();
                return Redirect::back()->withInput()->with(array('mesaj' => 'true', 'title' => 'Kaydetme Hatası', 'text' => 'Bu İstasyon Adı Zaten Mevcut!', 'type' => 'warning'));
            }
            if (Input::has('sayacadi')) {
                $sayacadlari = Input::get('sayacadi');
                $adlar = "";
                foreach ($sayacadlari as $sayacadi) {
                    $adlar .= ($adlar == "" ? "" : ",") . $sayacadi;
                }
                $istasyon->sayacadlari = $adlar;
            }
            $istasyon->istasyonadi = $adi;
            $istasyon->sayacsayi = $sayi;
            $istasyon->save();
            BackendController::IslemEkle(1, Auth::user()->id, 'label-success', 'fa-plus-circle', $adi . ' İsimli İstasyona Ait Bilgiler Eklendi.', 'Ekleyen:' . Auth::user()->adi_soyadi . ',İstasyon Numarası:' . $istasyon->id);
            DB::commit();
            return Redirect::to($this->servisadi.'/istasyon')->with(array('mesaj' => 'true', 'title' => 'İstasyon Bilgisi Eklendi', 'text' => 'İstasyon Bilgisi Başarıyla Eklendi', 'type' => 'success'));
        } catch (Exception $e) {
            DB::rollBack();
            Log::error($e);
            return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'İstasyon Bilgisi Eklenemedi', 'text' => 'İstasyon Bilgisi Eklenirken Hata ile Karşılaşıldı.', 'type' => 'error'));
        }
    }

    public function getIstasyonduzenle($id) {
        $istasyon = Istasyon::find($id);
        $sayacadlari = SayacAdi::where('sayactur_id',$this->servisid)->get();
        return View::make($this->servisadi.'.istasyonduzenle',array('istasyon'=>$istasyon,'sayacadlari'=>$sayacadlari))->with(array('title'=>$this->servisadi.' İstasyon Düzenleme Ekranı'));
    }

    public function postIstasyonduzenle($id) {
        try {
            $rules = ['istasyonadi' => 'required', 'sayacsayi' => 'required', 'sayacadi' => 'required'];
            $validate = Validator::make(Input::all(), $rules);
            $messages = $validate->messages();
            ...
            DB::beginTransaction();
            $istasyon = Istasyon::find($id);
            $bilgi = clone $istasyon;
            $adi = Input::get('istasyonadi');
            $sayi = Input::get('sayacsayi');
            if(Istasyon::where('istasyonadi',$adi)->where('id','<>',$id)->first()){
                Input::flash();
                return Redirect::back()->withInput()->with(array('mesaj' => 'true', 'title' => 'Kaydetme Hatası', 'text' => 'Bu İstasyon Adı Zaten Mevcut!', 'type' => 'warning'));
            }
            if (Input::has('sayacadi')) {
            ...
            $istasyon->sayacsayi = $sayi;
            $istasyon->save();
            BackendController::IslemEkle(2, Auth::user()->id, 'label-warning', 'fa-edit', $bilgi->istasyonadi . ' İsimli İstasyona Ait Bilgiler Güncellendi.', 'Güncelleyen:' . Auth::user()->adi_soyadi . ',İstasyon Numarası:' . $bilgi->id);
            DB::commit();
            return Redirect::to($this->servisadi.'/istasyon')->with(array('mesaj' => 'true', 'title' => 'İstasyon Bilgisi Güncellendi', 'text' => 'İstasyon Bilgisi Başarıyla Güncellendi', 'type' => 'success'));
        } catch (Exception $e) {
            DB::rollBack();
            Log::error($e);
            return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'İstasyon Bilgisi Güncellenemedi', 'text' => 'İstasyon Bilgisi Güncellenirken Hata ile Karşılaşıldı.', 'type' => 'error'));
        }
    }

    public function getIstasyonsil($id){
        try {
            DB::beginTransaction();
            $istasyon = Istasyon::find($id);
            if($istasyon){
                $kalibrasyon = Kalibrasyon::where('istasyon_id', $id)->first();
                if ($kalibrasyon) {
                    return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'İstasyon Bİlgisi Silinemez', 'text' => 'İstasyon Bilgisi Kalibrasyonda Kullanılmış.', 'type' => 'error'));
                }
                $bilgi=$istasyon;
                $istasyon->delete();
                BackendController::IslemEkle(3, Auth::user()->id, 'label-danger', 'fa-close', $bilgi->istasyonadi . ' İsimli İstasyona Ait Bilgiler Silindi.', 'Silen:' . Auth::user()->adi_soyadi . ',İstasyon Numarası:' . $bilgi->id);
                DB::commit();
                return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'İstasyon Bİlgisi Silindi', 'text' => 'İstasyon Bilgisi Başarıyla Silindi.', 'type' => 'success'));
            }else{
                return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'İstasyon Bİlgisi Silinemedi', 'text' => 'İstasyon Bilgisi Zaten Silinmiş.', 'type' => 'error'));
            }
        } catch (Exception $e) {
            DB::rollBack();
            Log::error($e);
            return Redirect::back()->with(array('mesaj' => 'true', 'title' => 'İstasyon Bİlgisi Silinemedi', 'text' => 'İstasyon Bilgisi Silinirken Sorun Oluştu.', 'type' => 'error'));
        }
    }
